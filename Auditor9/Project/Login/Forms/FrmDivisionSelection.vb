Public Class FrmDivisionSelection
    ...
    Public Sub MoveRec()
        ...
        If Not (AgL.StrCmp(AgL.PubUserName, "SA") Or AgL.StrCmp(AgL.PubUserName, AgLibrary.ClsConstant.PubSuperUserName)) Then
            ...
            mCondStr = mCondStr & " AND D.Div_Code In (" & Replace(StrUserDivisionList, "|", "'") & ")"
        End If

        DTTemp = AgL.FillData("Select D.Div_Code,Sg.DispName as Div_Name From Division D Left Join Subgroup Sg On D.Subcode = Sg.Subcode " & mCondStr, AgL.GcnMain).TABLES(0)
    ...
    Private Sub FGMain_BackgroundColorChanged(sender As Object, e As EventArgs) Handles FGMain.BackgroundColorChanged
