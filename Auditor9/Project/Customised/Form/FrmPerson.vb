...
#End Region
    ...
    Private Sub FrmQuality1_BaseFunction_MoveRec(ByVal SearchCode As String) Handles Me.BaseFunction_MoveRec
        ...
        mQry = "Select DiscountPer from ItemGroupPerson With (NoLock) Where Person = '" & mSearchCode & "' And ItemCategory Is Null And ItemGroup Is Null "
        DsTemp = AgL.FillData(mQry, AgL.GCn)
        If DsTemp.Tables(0).Rows.Count > 0 Then
    ...
    Public Overrides Sub ProcSave()
        ...
        Try
            ...
            mQry = "Select Count(*) from ItemGroupPerson With (NoLock) Where Person = '" & mSearchCode & "' And ItemCategory Is Null And ItemGroup Is Null "
            If AgL.Dman_Execute(mQry, IIf(AgL.PubServerName = "", AgL.GCn, AgL.GcnRead)).ExecuteScalar() > 0 Then
                mQry = "Update ItemGroupPerson Set DiscountPer=" & Val(Dgl1.Item(Col1Value, rowDiscount).Value) & "  Where Person = '" & mSearchCode & "' And ItemCategory Is Null And ItemGroup Is Null  "
                AgL.Dman_ExecuteNonQry(mQry, AgL.GCn, AgL.ECmd)
            Else
                If Val(Dgl1.Item(Col1Value, rowDiscount).Value) > 0 Then
                    mQry = "Insert Into ItemGroupPerson (Person, DiscountPer) Values (" & AgL.Chk_Text(mSearchCode) & ", " & Val(Dgl1.Item(Col1Value, rowDiscount).Value) & ") "
                    AgL.Dman_ExecuteNonQry(mQry, AgL.GCn, AgL.ECmd)
                End If
            End If
    ...
    Private Sub FrmPerson_BaseEvent_Data_Validation(ByRef passed As Boolean) Handles Me.BaseEvent_Data_Validation
        ...
        If Dgl1.Item(Col1Value, rowSalesTaxNo).Value <> "" Then
            If Dgl1.Item(Col1Value, rowSalesTaxNo).Value.ToString.Substring(0, 2) <> "88" Then
