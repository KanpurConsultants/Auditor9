...
#Region "Form Designer Code"
    Private Sub InitializeComponent()
        ...
        Me.TxtReferenceNo.Location = New System.Drawing.Point(351, 32)
        ...
        Me.LblReferenceNo.Size = New System.Drawing.Size(101, 16)
        Me.LblReferenceNo.TabIndex = 728
        Me.LblReferenceNo.Text = "Manual Ref. No."
        '
        'GrpDirectChallan
        '
...
#End Region
    ...
    Private Sub TempPhysicalStockEntry_BaseEvent_Approve_InTrans(ByVal SearchCode As String, ByVal Conn As Object, ByVal Cmd As Object) Handles Me.BaseEvent_Approve_InTrans
        ...
        With Dgl1
            For I = 0 To .RowCount - 1
                If .Item(Col1Item, I).Value <> "" Then
                    mSr += 1
                    mQry = "INSERT INTO StockHeadDetail(DocId, TSr, Sr, Item_Uid, Item, Process, " &
                            " Status, CurrentStock, Qty, Unit, " &
                            " MeasurePerPcs, TotalMeasure, CurrentStockMeasure, MeasureUnit,  " &
                            " Rate, Amount, ReferenceDocId, ReferenceDocIdSr, DifferenceQty, DifferenceMeasure) " &
                            " VALUES('" & mSearchCode & "', " & Val(mSr) & ", " & Val(mSr) & ", " &
    ...
    Private Sub FCreateHelpItem()
        ...
        Else
            mQry = "SELECT H.Code, H.Description, H.Unit, H.ItemType, H.SalesTaxPostingGroup, Measure, MeasureUnit , " &
                ...
                " Null As ReferenceDocId, Null As ReferenceDocIdSr, Null As PurchaseNO " &
                " FROM Item H " &
    ...
    Private Function FRetItemStock(ByVal ItemCode As String, ByVal AsOnDate As String, Optional ByVal CondStr As String = "") As Double
        Try
            mQry = " SELECT IfNull(Sum(Qty_Rec),0) - IfNull(Sum(Qty_Iss),0) " &
                    " FROM Stock " &
                    " WHERE Item = '" & ItemCode & "' And Date(V_Date) <= '" & AsOnDate & "'   " & CondStr
    ...
    'Private Sub FGroupOnItem(ByVal Conn As Object, ByVal Cmd As Object)
    ...
    Private Sub FrmPhysicalStockEntry_BaseEvent_ApproveDeletion_InTrans(ByVal SearchCode As String, ByVal Conn As Object, ByVal Cmd As Object) Handles Me.BaseEvent_ApproveDeletion_InTrans
        mQry = " Delete From Stock Where DocId = '" & mSearchCode & "'"
        AgL.Dman_ExecuteNonQry(mQry, Conn, Cmd)
    End Sub
    ...
    Private Sub FGroupOnItem(ByVal Conn As Object, ByVal Cmd As Object)
        ...
        mQry = " INSERT INTO Stock (DocId, TSr, Sr, " &
                ...
                " MeasureUnit, " &
                " Rate, Amount) " &
                " SELECT Max(L.DocId) As DocId, Row_Number() Over (Order By L.Item) As TSr, Row_Number() Over (Order By L.Item) As Sr, " &
                " Max(H.V_Type) As V_Type, Max(H.V_Prefix) As V_Prefix, Max(H.V_Date) As V_Date, " &
                " Max(H.V_No) As V_No,   " &
                " Max(H.V_No) As RecID,   " &
                " Max(H.Div_Code) As Div_Code,   " &
                " Max(H.Site_Code) As Site_Code,   " &
                " Max(H.FromGodown) As Godown,   " &
                " L.Item, " &
                " Sum(Case When IfNull(L.DifferenceQty,0) > 0 Then  IfNull(Abs(L.DifferenceQty),0) Else 0 End) As Qty_Iss, " &
                " Sum(Case When IfNull(L.DifferenceQty,0) < 0 Then  IfNull(Abs(L.DifferenceQty),0) Else 0 End) As Qty_Rec, " &
                " Max(L.Unit) As Unit, Max(L.MeasurePerPcs) As MeasurePerPcs, " &
                " Sum(Case When IfNull(L.DiffernceMeasure,0) > 0 Then  IfNull(Abs(L.DiffernceMeasure),0) Else 0 End) As Measure_Iss, " &
                " Sum(Case When IfNull(L.DiffernceMeasure,0) < 0 Then  IfNull(Abs(L.DiffernceMeasure),0) Else 0 End) As Measure_Rec, " &
                " Max(L.MeasureUnit) As MeasureUnit, " &
                " Max(L.Rate) As Rate, Sum(L.Amount) As Amount " &
                " FROM StockHeadDetail L  " &
        ...
        mQry = " INSERT INTO JobIssRecUID (DocID, TSr, Sr, Process, Item, Item_UID, ISSREC) " &
                " SELECT DocID, Sr AS TSr, Sr, 'PHSIC', Item, Item_UID, 'I' AS ISSREC   FROM StockHeadDetail  " &
                " WHERE DocID = '" & mSearchCode & "' AND Item_UID IS NOT NULL " &
                " AND DifferenceQty > 0 "
        AgL.Dman_ExecuteNonQry(mQry, Conn, Cmd)
    End Sub
