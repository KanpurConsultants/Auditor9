...
Public Class FrmUser
    ...
    Private Sub Topctrl1_tbSave() Handles Topctrl1.tbSave
        ...
        Try
            ...
            If Topctrl1.Mode = "Add" Then
                mQry = "Insert Into UserMast (Code, USER_NAME, PASSWD, Description, Admin, UpLoadDate,RowID) Values(" &
                        " '" & mCode & "', '" & mSearchCode & "', " & AgL.Chk_Text(AgL.CODIFY(TxtPassword.Text)) & "," &
                        " " & AgL.Chk_Text(TxtDescription.Text) & ",'" & AgL.MidStr(TxtAdminYn.Text, 0, 1) & "', " & AgL.ConvertDate(mUploadDate) & ",1) "
    ...
    Private Sub DGL1_CellContentClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DGL1.CellContentClick
        ...
        End Try
    End Sub
    Private Sub FHPGD_Site(mRowIndex As Integer)
        Dim mQry$
        ...
        FRH_Multiple.FFormatColumn(1, "Code", 50, DataGridViewContentAlignment.MiddleLeft)
        ...
        If FRH_Multiple.BytBtnValue = 0 Then
            DGL1(Col1SiteList, mRowIndex).Tag = FRH_Multiple.FFetchData(1, "|", "|", ",", True)
            DGL1(Col1SiteList, mRowIndex).Value = FRH_Multiple.FFetchData(1, "|", "|", ",", True)
        End If

        FRH_Multiple = Nothing
    End Sub

    Private Sub FHPGD_Division(mRowIndex As Integer)
        Dim mQry$
        Dim FRH_Multiple As DMHelpGrid.FrmHelpGrid_Multi
        Dim StrRtn As String = ""
        ...
        FRH_Multiple.FFormatColumn(0, "Tick", 40, DataGridViewContentAlignment.MiddleCenter, True)
        FRH_Multiple.FFormatColumn(1, "Code", 50, DataGridViewContentAlignment.MiddleLeft)
        FRH_Multiple.FFormatColumn(2, "Division Name", 150, DataGridViewContentAlignment.MiddleLeft)

        FRH_Multiple.StartPosition = FormStartPosition.CenterScreen
        FRH_Multiple.ShowDialog()

        If FRH_Multiple.BytBtnValue = 0 Then
            DGL1(Col1DivisionList, mRowIndex).Tag = FRH_Multiple.FFetchData(1, "|", "|", ",", True)
            DGL1(Col1DivisionList, mRowIndex).Value = FRH_Multiple.FFetchData(1, "|", "|", ",", True)
        End If
