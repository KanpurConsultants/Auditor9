...
Public Class FrmTag
    ...
    Dim rowNCat As Integer = 1
    Dim rowVoucherType As Integer = 2
...
#End Region
    Private Sub FrmYarn_BaseEvent_Data_Validation(ByRef passed As Boolean) Handles Me.BaseEvent_Data_Validation
        If DglMain.Item(Col1Value, rowDescription).Value.Trim = "" Then Err.Raise(1, , "Description Is Required!")

        If Topctrl1.Mode = "Add" Then
            mQry = "Select count(*) From Tag Where Description = '" & DglMain.Item(Col1Value, rowDescription).Value & "' "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then Err.Raise(1, , "Description Already Exist!")
        Else
            mQry = "Select count(*) From Tag Where Description = '" & DglMain.Item(Col1Value, rowDescription).Value & "' And Code <> '" & mInternalCode & "'"
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then Err.Raise(1, , "Description Already Exist!")
        End If
    End Sub
    Public Overridable Sub FrmYarn_BaseEvent_FindMain() Handles Me.BaseEvent_FindMain
        Dim mConStr$ = ""
        AgL.PubFindQry = "Select H.Description, Vt.Description As VoucherType, H.NCat
                         From Tag H 
    ...
    Private Sub FrmYarn_BaseEvent_Save_InTrans(ByVal SearchCode As String, ByVal Conn As Object, ByVal Cmd As Object) Handles Me.BaseEvent_Save_InTrans
        ...
        mQry = "UPDATE Tag
                ...
                V_Type = " & AgL.Chk_Text(DglMain.Item(Col1Value, rowVoucherType).Tag) & " 
    ...
    Private Sub FrmQuality1_BaseFunction_MoveRec(ByVal SearchCode As String) Handles Me.BaseFunction_MoveRec
        ...
        mQry = "Select H.*, Vt.Description As V_TypeDesc
                 From Tag H 
                 LEFT JOIN Voucher_Type Vt On H.V_Type = Vt.V_Type
        ...
        With DsTemp.Tables(0)
            If .Rows.Count > 0 Then
                ...
                DglMain.Item(Col1Value, rowDescription).Value = AgL.XNull(DsTemp.Tables(0).Rows(0)("Description"))
                DglMain.Item(Col1Value, rowNCat).Value = AgL.XNull(DsTemp.Tables(0).Rows(0)("NCat"))
                DglMain.Item(Col1Value, rowVoucherType).Tag = AgL.XNull(DsTemp.Tables(0).Rows(0)("V_Type"))
                DglMain.Item(Col1Value, rowVoucherType).Value = AgL.XNull(DsTemp.Tables(0).Rows(0)("V_TypeDesc"))
            End If
        End With
    End Sub
    Private Sub Topctrl1_tbEdit() Handles Topctrl1.tbEdit
        DglMain.CurrentCell = DglMain.Item(Col1Value, rowDescription)
        DglMain.Focus()
    End Sub
    ...
    End Sub
    Private Sub FrmTag_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    ...
    Private Sub FrmItemMaster_BaseEvent_Topctrl_tbEdit(ByRef Passed As Boolean) Handles Me.BaseEvent_Topctrl_tbEdit
        ...
        End If
    End Sub
    Private Sub FrmTag_BaseEvent_Topctrl_tbAdd() Handles Me.BaseEvent_Topctrl_tbAdd
        Try
            DglMain.CurrentCell = DglMain.Item(Col1Value, DglMain.FirstDisplayedCell.RowIndex)
            DglMain.Focus()
        Catch ex As Exception
            MsgBox(ex.Message & " [FrmTag_BaseEvent_Topctrl_tbAdd]")
        End Try
    End Sub
    Private Sub FrmTag_BaseFunction_IniGrid() Handles Me.BaseFunction_IniGrid
        ...
        DglMain.Item(Col1Head, rowVoucherType).Value = hcVoucherType
    ...
    End Sub
    Private Sub FrmTag_BaseFunction_BlankText() Handles Me.BaseFunction_BlankText
        Dim i As Integer
            ...
            Select Case DglMain.CurrentCell.RowIndex
                Case rowDescription
                    If DglMain.Item(Col1Head, DglMain.CurrentCell.RowIndex).Tag Is Nothing Then
                        mQry = "Select Code, Description As Name From Tag "
                ...
                Case rowNCat
                    If DglMain.Item(Col1Head, DglMain.CurrentCell.RowIndex).Tag Is Nothing Then
                        mQry = "SELECT DISTINCT Vt.NCat AS Code, Vt.NCat AS Description  FROM Voucher_Type Vt  "
                        DglMain.Item(Col1Head, DglMain.CurrentCell.RowIndex).Tag = AgL.FillData(mQry, AgL.GCn)
                    End If
                    If DglMain.AgHelpDataSet(Col1Value) Is Nothing Then
                        DglMain.AgHelpDataSet(Col1Value) = DglMain.Item(Col1Head, DglMain.CurrentCell.RowIndex).Tag
                ...
                Case rowVoucherType
                    If DglMain.Item(Col1Head, DglMain.CurrentCell.RowIndex).Tag Is Nothing Then
                        mQry = "SELECT Vt.V_Type, Vt.Description FROM Voucher_Type Vt  "
                        DglMain.Item(Col1Head, DglMain.CurrentCell.RowIndex).Tag = AgL.FillData(mQry, AgL.GCn)
                    End If
                    If DglMain.AgHelpDataSet(Col1Value) Is Nothing Then
                        DglMain.AgHelpDataSet(Col1Value) = DglMain.Item(Col1Head, DglMain.CurrentCell.RowIndex).Tag
    ...
    Private Sub Dgl1_CellEnter(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DglMain.CellEnter
        ...
        End Try
    End Sub
    Private Sub FrmTag_BaseEvent_Save_PostTrans(SearchCode As String) Handles Me.BaseEvent_Save_PostTrans
        ClsMain.FCreateItemDataTable()
    End Sub
    Private Sub LblIsSystemDefine_Click(sender As Object, e As EventArgs)
    End Sub
    Private Sub FrmTag_BaseEvent_Topctrl_tbRef() Handles Me.BaseEvent_Topctrl_tbRef
        Dim I As Integer
        For I = 0 To DglMain.Rows.Count - 1
