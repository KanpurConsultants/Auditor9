...
Public Class FrmFacilityFee
    ...
    Dim rowFacility As Integer = 0
    Dim rowFeeStructureName As Integer = 1
    Dim rowRecurrence As Integer = 2

    Public Const hcFacility As String = "Facility"
    Public Const hcFeeStructureName As String = "Fee Structure Name"
    Public Const hcRecurrence As String = "Recurrence"
...
#End Region
    ...
    Private Sub FrmYarn_BaseEvent_Data_Validation(ByRef passed As Boolean) Handles Me.BaseEvent_Data_Validation
        ...
        For I = 0 To Dgl1.Rows.Count - 1
            If Dgl1.Item(Col1SubHead, I).Value <> "" Then
    ...
    Public Overridable Sub FrmYarn_BaseEvent_FindMain() Handles Me.BaseEvent_FindMain
        ...
        AgL.PubFindQry = "SELECT H.Code, H.Description as Name
                            FROM Item H
                            WHERE H.V_Type =" & AgL.Chk_Text(ClsSchool.ItemV_Type_FacilityFee) & " "
    ...
    Private Sub FrmYarn_BaseEvent_Save_InTrans(ByVal SearchCode As String, ByVal Conn As Object, ByVal Cmd As Object) Handles Me.BaseEvent_Save_InTrans
        ...
        mQry = "UPDATE Item 
                Set 
                V_Type = '" & ClsSchool.ItemV_Type_FacilityFee & "',
                ItemType = '" & ClsSchool.ItemV_Type_FacilityFee & "',
                Specification = " & AgL.Chk_Text(DglMain.Item(Col1Value, rowFacility).Value) & ",
        ...
        For I = 0 To Dgl1.Rows.Count - 1
            If Dgl1.Item(Col1SubHead, I).Value <> "" Then
                ...
                mQry = "INSERT INTO FeeStructureRecurrence (Code, Sr, Fee, SubHead, Recurrence, Narration, 
                        Amount, DueDate, Comp_Code, Div_Code, Site_Code)
                        Select '" & SearchCode & "', " & mSr & ", 
                        " & AgL.Chk_Text(DglMain.Item(Col1Value, rowFacility).Tag) & ", 
                        " & AgL.Chk_Text(Dgl1.Item(Col1SubHead, I).Tag) & ",
                        " & AgL.Chk_Text(DglMain.Item(Col1Value, rowRecurrence).Value) & ",  
        ...
        If AgL.XNull(Dgl1.Item(Col1SubHead, mRow).Tag) = "" Then
            ...
            If AgL.XNull(Dgl1.Item(Col1SubHead, mRow).Tag) = "" Then
                ...
                mQry = "INSERT INTO SubGroup(SubCode, SubGroupType, Name, DispName, Parent) " &
                    ...
                    " " & AgL.Chk_Text(ClsSchool.SubGroupType_FacilityHead) & ", " &
            ...
            Else
                mQry = " UPDATE SubGroup 
                        ...
                        Parent = " & AgL.Chk_Text(DglMain.Item(Col1Value, rowFacility).Tag) & "
    ...
    Private Sub FrmQuality1_BaseFunction_MoveRec(ByVal SearchCode As String) Handles Me.BaseFunction_MoveRec
        ...
        Dim I As Integer
        mQry = "SELECT H.*, Facility.Name As FacilityName, SubHead.Name As SubHeadName
                FROM FeeStructureRecurrence H
                LEFT JOIN SubGroup Facility ON H.Fee = Facility.SubCode
        ...
        With DsTemp.Tables(0)
            ...
            If .Rows.Count > 0 Then
                For I = 0 To DsTemp.Tables(0).Rows.Count - 1
                    ...
                    DglMain.Item(Col1Value, rowFacility).Tag = AgL.XNull(.Rows(0)("Fee"))
                    DglMain.Item(Col1Value, rowFacility).Value = AgL.XNull(.Rows(0)("FacilityName"))
                    Dgl1.Item(Col1SubHead, I).Tag = AgL.XNull(.Rows(I)("SubHead"))
                    Dgl1.Item(Col1SubHead, I).Value = AgL.XNull(.Rows(I)("SubHeadName"))
                    DglMain.Item(Col1Value, rowRecurrence).Value = AgL.XNull(.Rows(I)("Recurrence"))
    ...
    Private Sub FrmYarn_BaseFunction_FIniMast(ByVal BytDel As Byte, ByVal BytRefresh As Byte) Handles Me.BaseFunction_FIniMast
        ...
        mQry = "SELECT I.Code AS SearchCode FROM Item I  
                WHERE I.V_Type =  '" & ClsSchool.ItemV_Type_FacilityFee & "'" &
                " Order By I.Code "
    ...
    Private Sub FrmItemBOM_BaseEvent_Topctrl_tbAdd() Handles Me.BaseEvent_Topctrl_tbAdd
        If DglMain.Rows(rowFacility).Visible Then DglMain.CurrentCell = DglMain(Col1Value, rowFacility)
        DglMain.Focus()
    End Sub
    Private Sub FrmItemBOM_BaseFunction_IniGrid() Handles Me.BaseFunction_IniGrid
        ...
        DglMain.Item(Col1Head, rowFacility).Value = hcFacility
        DglMain.Item(Col1Head, rowFeeStructureName).Value = hcFeeStructureName
        DglMain.Item(Col1Head, rowRecurrence).Value = hcRecurrence
    ...
    Private Sub DglRateType_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Dgl1.KeyDown
        ...
        If e.Control And e.KeyCode = Keys.D Then
            If IsFeeStructureChangeAllowed(Dgl1.Item(Col1SubHead, Dgl1.CurrentCell.RowIndex).Tag) = False Then
                MsgBox("Students exists for this Facility. Can't change fee structure.", MsgBoxStyle.Information)
                Exit Sub
            Else
        ...
        If mColumn = DglMain.Columns(Col1Value).Index Then
            ...
            Select Case mRow
                Case rowFacility
                    ...
                    DglMain.Item(Col1Value, rowFeeStructureName).Value = DglMain.Item(Col1Value, rowFacility).Value + "-" + bCompYear + "-" + AgL.PubSiteName + "-" + AgL.PubDivName
            End Select
        End If
        Calculation()
    End Sub
    ...
    Private Sub FrmSaleOrder_BaseFunction_Calculation() Handles Me.BaseFunction_Calculation
        ...
        If Topctrl1.Mode = "Browse" Then Exit Sub
    End Sub
    Private Sub FrmFacilityFee_BaseEvent_Topctrl_tbRef() Handles Me.BaseEvent_Topctrl_tbRef
    ...
    Private Sub Dgl1_CellBeginEdit(sender As Object, e As DataGridViewCellCancelEventArgs) Handles Dgl1.CellBeginEdit
        Try
            ...
            Select Case Dgl1.Columns(Dgl1.CurrentCell.ColumnIndex).Name
                Case Col1SubHead, Col1DueDate
                    If AgL.XNull(Dgl1.Item(Col1SubHead, Dgl1.CurrentCell.RowIndex).Tag) <> "" Then
                        If IsFeeStructureChangeAllowed(Dgl1.Item(Col1SubHead, Dgl1.CurrentCell.RowIndex).Tag) = False Then
                            MsgBox("Students exists for this Facility. Can't change fee structure.", MsgBoxStyle.Information)
                            e.Cancel = True
                            Exit Sub
    ...
    Private Sub DglMain_CellBeginEdit(sender As Object, e As DataGridViewCellCancelEventArgs) Handles DglMain.CellBeginEdit
        ...
        End Try
    End Sub
    Private Function IsFeeStructureChangeAllowed(SubHead As String) As Boolean
        IsFeeStructureChangeAllowed = True
        If Not AgL.StrCmp(Topctrl1.Mode, "Add") Then
            mQry = " Select Count(*) From SubgroupFacility 
                    Where Facility = '" & DglMain.Item(Col1Value, rowFacility).Tag & "'"
            If SubHead <> "" Then mQry += " And FacilitySubHead = '" & SubHead & "'"
