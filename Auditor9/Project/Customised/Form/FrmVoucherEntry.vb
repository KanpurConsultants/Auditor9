...
#End Region
    ...
    Private Sub FrmSaleOrder_BaseEvent_Save_InTrans(ByVal SearchCode As String, ByVal Conn As Object, ByVal Cmd As Object) Handles Me.BaseEvent_Save_InTrans
        ...
        If (LblV_Type.Tag = AgLibrary.ClsMain.agConstants.Ncat.DebitNote And TxtVoucherCategory.Text.ToUpper = AgLibrary.ClsMain.agConstants.VoucherCategory.Purchase) Or
                (LblV_Type.Tag = AgLibrary.ClsMain.agConstants.Ncat.CreditNote And TxtVoucherCategory.Text.ToUpper = AgLibrary.ClsMain.agConstants.VoucherCategory.Sales) Then
        ...
        For I = 0 To Dgl1.RowCount - 1
            If IIf(LblV_Type.Tag = Ncat.CreditNote Or LblV_Type.Tag = Ncat.DebitNote, Dgl1.Item(Col1Remark, I).Value, Dgl1.Item(Col1Subcode, I).Value) <> "" Then
        ...
        Try
            Select Case sender.NAME
                ...
                Case TxtPartyName.Name
                    If LblV_Type.Tag = Ncat.DebitNote Or LblV_Type.Tag = Ncat.CreditNote Then
    ...
    Private Sub ValidateSubcodeLine(mRowIndex As Integer)
        ...
        If Not (LblV_Type.Tag = Ncat.DebitNote Or LblV_Type.Tag = Ncat.CreditNote) Then
        ...
        If Dgl1.Columns(Col1HSN).Visible Then
            mQry = "Select Sg.HSN From Subgroup Sg Where Sg.Subcode = '" & Dgl1.Item(Col1Subcode, mRowIndex).Tag & "' "
            DtTemp = AgL.FillData(mQry, AgL.GCn).Tables(0)
            If DtTemp.Rows.Count > 0 Then
    ...
    Private Sub FValidate_ReferenceNo(RowNumber As Integer)
        If LblV_Type.Tag = AgLibrary.ClsMain.agConstants.Ncat.DebitNote Or LblV_Type.Tag = AgLibrary.ClsMain.agConstants.Ncat.CreditNote Then
    ...
    Private Sub FCreateHelpReferenceNoDebitNote()
        ...
        If Dgl1.AgHelpDataSet(Col1Specification) Is Nothing Then
            ...
            mQry = "select L.DocID || '+' || Cast(L.V_SNo as nVarchar) as SearchKey, L.DivCode || L.Site_Code || '-' || L.V_Type || '-' ||  L.RecId as InvoiceNo, L.V_Date, I.Description as ItemName, S.Qty_Rec + S.Qty_Iss as Qty, S.Rate
                ...
                Left Join Item I  With (NoLock) On S.Item = I.Code
                Left Join viewHelpSubgroup Sg  With (NoLock) On L.Subcode = Sg.Code
                where DivCode='" & AgL.PubDivCode & "' And  L.Subcode = '" & TxtPartyName.Tag & "' And L.V_Date<='" & CDate(TxtV_Date.Text).ToString("s") & "' 
    ...
    Private Sub FrmLedgerHeadDirect_BaseEvent_Topctrl_tbEdit(ByRef Passed As Boolean) Handles Me.BaseEvent_Topctrl_tbEdit
        ...
        bLockText = AgL.XNull(AgL.Dman_Execute(" Select LockText From LedgerHead With (NoLock) Where DocId = '" & mSearchCode & "'", AgL.GCn).ExecuteScalar())
        If bLockText <> "" Then
            If AgL.StrCmp(AgL.PubUserName, "Super") Or AgL.StrCmp(AgL.PubUserName, "Sa") Then
                If MsgBox(bLockText + " Do you want to continue.", MsgBoxStyle.Question + MsgBoxStyle.YesNo) = MsgBoxResult.No Then
                    Passed = False
                    Exit Sub
                End If
            Else
                MsgBox(bLockText, MsgBoxStyle.Information)
                Passed = False
                Exit Sub
            End If
        End If
