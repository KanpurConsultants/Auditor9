...
Public Class FrmStockHeader
    ...
    Public Sub FMoveRec(ByVal SearchCode As String)
        ...
        Try
            ...
            mQry = "SELECT H.*, Transporter.Name as TransporterName
                    FROM StockHeadTransport H  With (NoLock)                     
                    LEFT JOIN viewHelpSubgroup Transporter  With (NoLock) On H.Transporter = Transporter.Code 
    ...
    Public Sub FSave(ByVal SearchCode As String, ByVal Conn As Object, ByVal Cmd As Object)
        ...
        If (Dgl1.Item(Col1Value, rowTransporter).Tag <> Nothing) And (Dgl1.Item(Col1Value, rowLrNo).Value <> Nothing) Then
            ...
            Else
                mQry = "Select count(*) From StockHeadTransport Where Transporter ='" & Dgl1.Item(Col1Value, rowTransporter).Tag & "' AND LrNo = '" & Dgl1.Item(Col1Value, rowLrNo).Value & "' And DocID <>'" & SearchCode & "'"
        ...
        mQry = "Delete From StockHeadTransport Where DocId = '" & SearchCode & "'"
        ...
        If Dgl1.Item(Col1Value, rowTransporter).Tag <> "" Or
           ...
           Dgl1.Item(Col1Value, rowRoadPermitDate).Value <> "" Then
            ...
            mQry = "Insert Into StockHeadTransport (DocID, Transporter, LRNo, LRDate, NoOfBales, PrivateMark, Weight, Freight, PaymentType, RoadPermitNo, RoadPermitDate)
                Values ('" & SearchCode & "', 
            ...
            mQry = "Update StockHead Set UploadDate=Null Where DocId ='" & SearchCode & "'"
            AgL.Dman_ExecuteNonQry(mQry, Conn, Cmd)

            If ClsMain.IsScopeOfWorkContains("+Cloth Aadhat Module") Then
                Dim mFieldCount As Integer = 0
                mQry = "Select GenDocID From StockHead With (NoLock) Where DocId = '" & SearchCode & "' And GenDocID Is Not Null "
                dtTemp = AgL.FillData(mQry, IIf(AgL.PubServerName = "", Conn, AgL.GcnRead)).Tables(0)
                If dtTemp.Rows.Count > 0 Then
                    mQry = "Update StockHeadTransport Set "
    ...
    Private Sub FillLrEntries()
        ...
        '        LEFT JOIN StockHead Pi ON Pi.LrCode = Lr.Code
        ...
        mQry = "SELECT VLr.Code, Max(Lr.LRNo) AS LrNo, Max(Lr.LRDate) AS LrDate, Max(Lr.Weight) AS Weight, 
                ...
                WHERE H.SubCode = '" & mPartyCode & "'
                GROUP BY VLr.Code
                HAVING Count(*) > (SELECT Count(DocId) FROM StockHead WHERE LrCode = VLr.Code) "
