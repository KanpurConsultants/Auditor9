...
Public Class FrmStockEntryMultiLineUI
    Dim mQry As String = ""
    ...
    Public Sub IniGrid(DocID As String)
        ...
        With AgCL
            ...
            .AddAgNumberColumn(Dgl1, Col1Rate, 100, 8, 2, False, Col1Rate, False, False, True)
        End With
        ...
        Try
            ...
            Select Case Dgl1.Columns(Dgl1.CurrentCell.ColumnIndex).Name
                Case Col1Size
                    ...
                    Dgl1.Item(Col1Rate, mRowIndex).Value = ClsMain.FGetRateWithRatePattern("", mPartyCode, mSettingGroup, mDivCode, mSiteCode, Process.Purchase, mVType, mRateType,
                        AgL.XNull(DglRow.Cells(FrmStockEntry.Col1ItemCategory).Tag),
                        AgL.XNull(DglRow.Cells(FrmStockEntry.Col1ItemGroup).Tag),
                        AgL.XNull(DglRow.Cells(FrmStockEntry.Col1Item).Tag),
                        AgL.XNull(DglRow.Cells(FrmStockEntry.Col1Dimension1).Tag),
                        AgL.XNull(DglRow.Cells(FrmStockEntry.Col1Dimension2).Tag),
                        AgL.XNull(DglRow.Cells(FrmStockEntry.Col1Dimension3).Tag),
                        AgL.XNull(DglRow.Cells(FrmStockEntry.Col1Dimension4).Tag),
                    ...
                    If Val(Dgl1.Item(Col1Rate, mRowIndex).Value) > 0 Then
                        Dgl1.Item(Col1Rate, mRowIndex).Value = Val(Dgl1.Item(Col1Rate, mRowIndex).Value) +
                            ClsMain.FGetRateWithRatePattern(RateCategory.RateAddition, mPartyCode, mSettingGroup, mDivCode, mSiteCode, Process.Purchase, mVType, mRateType,
                                AgL.XNull(DglRow.Cells(FrmStockEntry.Col1ItemCategory).Tag),
                                AgL.XNull(DglRow.Cells(FrmStockEntry.Col1ItemGroup).Tag),
                                AgL.XNull(DglRow.Cells(FrmStockEntry.Col1Item).Tag),
                                AgL.XNull(DglRow.Cells(FrmStockEntry.Col1Dimension1).Tag),
    ...
    Private Sub FCreateHelpSize(RowIndex As Integer)
        ...
        If AgL.XNull(DglRow.Cells(FrmStockEntry.Col1ItemCategory).Tag) <> "" Then
            strCond += " And (I.ItemCategory = '" & AgL.XNull(DglRow.Cells(FrmStockEntry.Col1ItemCategory).Tag) & "' 
        ...
        If DtItemRelation.Rows.Count > 0 Then
            If AgL.XNull(DglRow.Cells(FrmStockEntry.Col1Dimension3).Tag) <> "" Then
                If DtItemRelation.Select("ItemV_Type = '" & ItemV_Type.Dimension3 & "'
                                And RelatedItemV_Type = '" & ItemV_Type.SIZE & "'").Length > 0 Then
                    Dim DrItemRelation As DataRow() = DtItemRelation.Select("Item = '" & AgL.XNull(DglRow.Cells(FrmStockEntry.Col1Dimension3).Tag) & "'")
