...
Public Class FrmPartyAcSettlement
...
#End Region
            ...
            If Dgl2.Item(Col2InvoiceNo, I).Value <> "" Then
                    ...
                    If Dgl2.Item(Col2Select, I).Value = "þ" Then
                        ...
                        If Dgl2.Item(Col2BtnAdjDetail, I).Tag IsNot Nothing Then
                            If ClsMain.IsScopeOfWorkContains(IndustryType.KiranaIndustry) Then
                                CType(Dgl2.Item(Col2BtnAdjDetail, I).Tag, FrmPartyAcSettlementInvoiceAdjKirana).FSave(mSearchCode, mSr, Conn, Cmd)
                            Else
                ...
                Else
                    If Dgl2.Rows(I).Visible = True And Dgl2.Item(Col2Select, I).Value = "þ" Then
                        ...
                        If Dgl2.Item(Col2BtnAdjDetail, I).Tag IsNot Nothing Then
                            If ClsMain.IsScopeOfWorkContains(IndustryType.KiranaIndustry) Then
                                CType(Dgl2.Item(Col2BtnAdjDetail, I).Tag, FrmPartyAcSettlementInvoiceAdjKirana).FSave(mSearchCode, Val(Dgl2.Item(ColSNo, I).Tag), Conn, Cmd)
                            Else
                        ...
                        For J = 0 To obj.Dgl1.Rows.Count - 1
                            If obj.Dgl1.Item(FrmPartyAcSettlementInvoiceAdj.Col1Head, J).Value <> "" And Val(obj.Dgl1.Item(FrmPartyAcSettlementInvoiceAdj.Col1Amount, J).Value) > 0 Then
                                ...
                                If obj.Dgl1.Item(FrmPartyAcSettlementInvoiceAdj.Col1AdditionDeduction, J).Value.ToString.ToUpper = "DEDUCTION" Then
                                    ...
                                    mLedgerPostingData += " Select " & AgL.Chk_Text(TxtParty.Tag) & " as Subcode, " & AgL.Chk_Text(TxtLinkedParty.Tag) & " As LinkedSubCode, " & AgL.Chk_Text(obj.Dgl1.Item(FrmPartyAcSettlementInvoiceAdj.Col1PostingAc, J).Value) & " as ContraAc, " & Val(obj.Dgl1.Item(FrmPartyAcSettlementInvoiceAdj.Col1Amount, J).Value) & " as AmtDr, 0 as AmtCr, '" & mNarration & "' as Narration, Null ChqNo, Null ChqDate  "
                                ...
                                Else
                                    ...
                                    mLedgerPostingData += " UNION ALL "
                                    mLedgerPostingData += " Select " & AgL.Chk_Text(TxtParty.Tag) & " as Subcode, " & AgL.Chk_Text(TxtLinkedParty.Tag) & " As LinkedSubCode, " & AgL.Chk_Text(obj.Dgl1.Item(FrmPartyAcSettlementInvoiceAdj.Col1PostingAc, J).Value) & " as ContraAc, 0 as AmtDr, " & Val(obj.Dgl1.Item(FrmPartyAcSettlementInvoiceAdj.Col1Amount, J).Value) & " as AmtCr, '" & mNarration & "' as Narration, Null ChqNo, Null ChqDate  "
                ...
                If Val(Dgl2.Item(Col2InvoiceAmount, I).Value) > 0 Then
                    If Dgl2.Item(Col2Select, I).Value = "þ" Then
                        If Dgl2.Item(Col2BtnAdjDetail, I).Tag IsNot Nothing Then
                            ...
                            If ClsMain.IsScopeOfWorkContains(IndustryType.KiranaIndustry) Then
                                FrmObj = CType(Dgl2.Item(Col2BtnAdjDetail, I).Tag, FrmPartyAcSettlementInvoiceAdjKirana)
                            Else
                                FrmObj = CType(Dgl2.Item(Col2BtnAdjDetail, I).Tag, FrmPartyAcSettlementInvoiceAdj)
                            End If
    ...
    Private Sub ShowSupplierSattlementAdjustment(mRow As Integer, Optional ShowDialog As Boolean = True)
        ...
        If Dgl2.Item(Col2BtnAdjDetail, mRow).Tag IsNot Nothing Then
            If ClsMain.IsScopeOfWorkContains(IndustryType.KiranaIndustry) Then
                FrmObj = CType(Dgl2.Item(Col2BtnAdjDetail, mRow).Tag, FrmPartyAcSettlementInvoiceAdjKirana)
            Else
                FrmObj = CType(Dgl2.Item(Col2BtnAdjDetail, mRow).Tag, FrmPartyAcSettlementInvoiceAdj)
            End If
        Else
            If ClsMain.IsScopeOfWorkContains(IndustryType.KiranaIndustry) Then
                FrmObj = New FrmPartyAcSettlementInvoiceAdjKirana
            Else
    ...
    Private Sub FManageAadhatCommission()
        ...
        For I = 0 To Dgl2.SelectedCells.Count - 1
            ...
            If Dgl2.Item(Col2Select, mRow).Value = "þ" Then
                ...
                CType(Dgl2.Item(Col2BtnAdjDetail, mRow).Tag, FrmPartyAcSettlementInvoiceAdj).Dgl1.Item(FrmPartyAcSettlementInvoiceAdj.Col1Amount, 0).Value = AgL.VNull(Dgl2.Item(Col2CommissionAmount, mRow).Value)
                CType(Dgl2.Item(Col2BtnAdjDetail, mRow).Tag, FrmPartyAcSettlementInvoiceAdj).Dgl1.Item(FrmPartyAcSettlementInvoiceAdj.Col1Remark, 0).Value = AgL.XNull(Dgl2.Item(Col2CommissionRemark, mRow).Value)
                CType(Dgl2.Item(Col2BtnAdjDetail, mRow).Tag, FrmPartyAcSettlementInvoiceAdj).Calculation()
                Dgl2.Item(Col2SettlementDeduction, mRow).Value = CType(Dgl2.Item(Col2BtnAdjDetail, mRow).Tag, FrmPartyAcSettlementInvoiceAdj).GetDeductions
            Else
                If Dgl2.Item(Col2BtnAdjDetail, mRow).Tag IsNot Nothing Then
                    CType(Dgl2.Item(Col2BtnAdjDetail, mRow).Tag, FrmPartyAcSettlementInvoiceAdj).Dgl1.Item(FrmPartyAcSettlementInvoiceAdj.Col1Amount, 0).Value = "0"
                    CType(Dgl2.Item(Col2BtnAdjDetail, mRow).Tag, FrmPartyAcSettlementInvoiceAdj).Dgl1.Item(FrmPartyAcSettlementInvoiceAdj.Col1Remark, 0).Value = ""
                    CType(Dgl2.Item(Col2BtnAdjDetail, mRow).Tag, FrmPartyAcSettlementInvoiceAdj).Calculation()
                    Dgl2.Item(Col2SettlementDeduction, mRow).Value = CType(Dgl2.Item(Col2BtnAdjDetail, mRow).Tag, FrmPartyAcSettlementInvoiceAdj).GetDeductions
                End If
            End If
    ...
    Private Sub FGetPrint(mPrintFor As ClsMain.PrintFor)
        ...
        mQry = "select H.Div_Code || H.Site_Code || '-' || H.V_Type || '-' || H.ManualRefNo as DocNo, H.V_Date, HSg.DispName as HSubcodeName, 
                ...
                Ld.LineTotalAmount As LineTotalAmount,
                ...
                LEFT JOIN (
                        Select L.DocId, Sum(L.Amount) As LineTotalAmount
                        From LedgerHeadDetail L 
                        Group By L.DocId) As Ld On H.DocId = Ld.DocId
    ...
    Private Sub FrmPartyAcSettlement_BaseEvent_Topctrl_tbMore() Handles Me.BaseEvent_Topctrl_tbMore
    ...
    Private Sub FrmPartyAcSettlement_BaseFunction_DispText() Handles Me.BaseFunction_DispText
        ...
        End If
    End Sub
    Private Sub FrmPartyAcSettlement_BaseEvent_Topctrl_tbDel(ByRef Passed As Boolean) Handles Me.BaseEvent_Topctrl_tbDel
        If ClsMain.IsEntryLockedWithLockText("LedgerHead", "DocId", mSearchCode) = True Then
            Passed = False
            Exit Sub
        End If
