...
Public Class ClsCustomerFairReport
    ...
    Private Sub ObjRepFormGlobal_ProcessReport() Handles ReportFrm.ProcessReport
        ProcCustomerFairReport()
    End Sub
    Public Sub New(ByVal mReportFrm As FrmRepDisplay)
        ReportFrm = mReportFrm
    End Sub
    Public Sub ProcCustomerFairReport(Optional mFilterGrid As AgControls.AgDataGrid = Nothing,
        ...
        Try
            ...
            Else
                mQry = "SELECT Site.Name as Site, H.V_Date AS BillDate, H.ManualRefNo AS BillNo, Sg.Name AS PartyName, Sit.LrNo AS BiltyNo, 
                    H.Net_Amount  AS BillAmount
                    FROM SaleInvoice H 
                    LEFT JOIN Voucher_Type Vt On H.V_Type = Vt.V_Type
                    LEFT JOIN ViewHelpSubgroup Sg ON H.BillToParty = Sg.Code
                    Left Join SiteMast Site On H.Site_Code = Site.Code
                    LEFT JOIN SaleInvoiceTransport Sit ON H.DocID = Sit.DocID  " & mCondStr
            End If
            DsHeader = AgL.FillData(mQry, AgL.GCn)

            If DsHeader.Tables(0).Rows.Count = 0 Then Err.Raise(1, , "No Records To Print!")

            ReportFrm.Text = "Customer Fair Report"
            ReportFrm.ClsRep = Me
            ReportFrm.ReportProcName = "ProcCustomerFairReport"
