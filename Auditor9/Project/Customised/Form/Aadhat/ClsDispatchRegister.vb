...
Public Class ClsDispatchRegister
    ...
    Private Sub ObjRepFormGlobal_ProcessReport() Handles ReportFrm.ProcessReport
        ProcDispatchRegister()
    End Sub
    Public Sub New(ByVal mReportFrm As FrmRepDisplay)
        ReportFrm = mReportFrm
    End Sub
    Public Sub ProcDispatchRegister(Optional mFilterGrid As AgControls.AgDataGrid = Nothing,
        ...
        Try
            ...
            mQry = " SELECT L.DocId As SearchCode, Max(H.ManualRefNo) AS InvoiceNo, strftime('%d/%m/%Y', Max(H.V_Date)) AS InvoiceDate, 
                    Max(Sg.Name) AS Party, 
                    Sum(H.Net_Amount) AS BillAmount,
                    Max(Supp.Name) AS Supplier, Sum(L.Amount) AS Amount, 
                    Sum(CASE WHEN I.Description LIKE '%Freight%' THEN L.Amount ELSE 0 END) AS Freight, 
                    Sum(CASE WHEN I.Description LIKE '%Packing%' THEN L.Amount ELSE 0 END) AS Packing,
                    Sum(L.Net_Amount) AS TotalAmount
                    FROM SaleInvoice H 
                    ...
                    WHERE Vt.NCat = '" & Ncat.SaleInvoice & "' " & mCondStr &
                    " GROUP BY L.DocID, I.ItemGroup "
            DsHeader = AgL.FillData(mQry, AgL.GCn)

            If DsHeader.Tables(0).Rows.Count = 0 Then Err.Raise(1, , "No Records To Print!")

            ReportFrm.Text = "Dispatch Report"
            ReportFrm.ClsRep = Me
            ReportFrm.ReportProcName = "ProcDispatchRegister"
