...
Public Class ClsSaleOrderStatusAadhat
    ...
    Public Sub ProcMain(Optional mFilterGrid As AgControls.AgDataGrid = Nothing,
        ...
        Try
            ...
            If ReportFrm.FGetText(0).ToString.ToUpper <> "ALL" Then
                mCondStr = mCondStr & " And IfNull(H.Status,'Active') = '" & ReportFrm.FGetText(0) & "' "
            End If
            mCondStr = mCondStr & " AND Date(H.V_Date) Between " & AgL.Chk_Date(ReportFrm.FGetText(1)) & " And " & AgL.Chk_Date(ReportFrm.FGetText(2)) & " "
            ...
            mCondStr = mCondStr & Replace(ReportFrm.GetWhereCondition("H.Site_Code", 6), "''", "'")
            mCondStr = mCondStr & Replace(ReportFrm.GetWhereCondition("H.Div_Code", 7), "''", "'")
            mCondStr = mCondStr & ReportFrm.GetWhereCondition("LTV.Agent", 8)
            mCondStr = mCondStr & ReportFrm.GetWhereCondition("City.CityCode", 9)
            mCondStr = mCondStr & ReportFrm.GetWhereCondition("City.State", 10)
            mCondStr = mCondStr & ReportFrm.GetWhereCondition("L.SalesRepresentative", 11)
            mCondStr = mCondStr & ReportFrm.GetWhereCondition("H.ResponsiblePerson", 12)

            If ReportFrm.FGetText(13) <> "All" Then
                mTags = ReportFrm.FGetText(13).ToString.Split(",")
