...
Public Class ClsSupplierFairReport
    ...
    Dim rowSalesRepresentative As Integer = 5
    ...
    Dim mHelpPartyQry$ = " Select 'o' As Tick,  Sg.SubCode As Code, Sg.DispName || ',' ||  City.CityName AS Party, Sg.Address FROM SubGroup Sg Left Join City On Sg.CityCode = City.CityCode Where Sg.Nature In ('Supplier','Supplier','Cash') "
    ...
    Public Sub Ini_Grid()
        Try
            ...
            ReportFrm.CreateHelpGrid("Report Type", "Report Type", FrmRepDisplay.FieldFilterDataType.StringType, FrmRepDisplay.FieldDataType.SingleSelection, mQry, "Detail")
            ...
            ReportFrm.CreateHelpGrid("SalesRepresentative", "Sales Representative", FrmRepDisplay.FieldFilterDataType.StringType, FrmRepDisplay.FieldDataType.MultiSelection, mHelpPartyQry,, 600, 650, 300)
        Catch ex As Exception
            MsgBox(ex.Message)
        End Try
    End Sub
    Private Sub ObjRepFormGlobal_ProcessReport() Handles ReportFrm.ProcessReport
        ProcSupplierFairReport()
    End Sub
    Public Sub New(ByVal mReportFrm As FrmRepDisplay)
        ReportFrm = mReportFrm
    End Sub
    Public Sub ProcSupplierFairReport(Optional mFilterGrid As AgControls.AgDataGrid = Nothing,
        ...
        Try
            ...
            mCondStr = " Where 1=1"
            mCondStr = mCondStr & " And Vt.NCat = '" & Ncat.PurchaseInvoice & "'"
            ...
            If ReportFrm.FGetText(rowReportType) = "Summary" Then
                mQry = "SELECT Max(Sg.Name) AS Party, Max(C.CityName) AS City, Sum(H.Net_Amount) AS Amount
                        FROM PurchInvoice H 
                        LEFT JOIN Voucher_Type Vt On H.V_Type = Vt.V_Type
                        LEFT JOIN ViewHelpSubgroup Sg ON H.BillToParty = Sg.Code
                        LEFT JOIN City C ON Sg.CityCode = C.CityCode " & mCondStr &
                        " GROUP BY H.BillToParty "
            Else
                mQry = "SELECT Site.Name as Site,H.V_Date AS BillDate, H.ManualRefNo AS BillNo, Sg.Name AS PartyName, Sit.LrNo AS BiltyNo, 
                    H.Gross_Amount AS GrossAmt, H.Tax1 + H.Tax2 + H.Tax3 AS Addition, H.Net_Amount  AS BillAmount
                    FROM PurchInvoice H 
                    LEFT JOIN Voucher_Type Vt On H.V_Type = Vt.V_Type
                    LEFT JOIN ViewHelpSubgroup Sg ON H.BillToParty = Sg.Code
                    Left Join SiteMast Site On H.Site_Code = Site.Code
                    LEFT JOIN PurchInvoiceTransport Sit ON H.DocID = Sit.DocID  " & mCondStr
            End If
            DsHeader = AgL.FillData(mQry, AgL.GCn)

            If DsHeader.Tables(0).Rows.Count = 0 Then Err.Raise(1, , "No Records To Print!")

            ReportFrm.Text = "Supplier Fair Report"
            ReportFrm.ClsRep = Me
            ReportFrm.ReportProcName = "ProcSupplierFairReport"
