...
Public Class FrmPersonWiseDiscount
    ...
    Public Const Col1DiscountPattern As String = "Pattern"
    Public Const Col1DiscountPer As String = "Discount @"
    Public Const Col1AdditionalDiscountPer As String = "Add. Discount @"
    ...
    Public Sub IniGrid(DocID As String)
        ...
        With AgCL
            ...
            .AddAgTextColumn(Dgl1, Col1DiscountPattern, 100, 255, Col1DiscountPattern, False, False)
            .AddAgNumberColumn(Dgl1, Col1DiscountPer, 90, 5, 0, False, "Pcs Less", True, False, True)
            .AddAgNumberColumn(Dgl1, Col1AdditionalDiscountPer, 90, 5, 0, False, "Discount", True, False, True)
            .AddAgNumberColumn(Dgl1, Col1AdditionPer, 90, 5, 0, False, Col1AdditionPer, True, False, True)
        End With
    ...
    Public Sub FMoverec(DocID As String)
        ...
        Dim I As Integer
        mQry = "Select L.ItemGroup, L.ItemCategory, L.DiscountCalculationPattern, L.DiscountPer, L.AdditionalDiscountPer, L.AdditionPer, IG.Description as ItemGroupName, IC.Description as ItemCategoryName
        ...
        With DsTemp.Tables(0)
            ...
            If .Rows.Count > 0 Then
                For I = 0 To DsTemp.Tables(0).Rows.Count - 1
                    ...
                    Dgl1.Item(Col1DiscountPattern, I).Value = AgL.XNull(.Rows(I)("DiscountCalculationPattern"))
                    Dgl1.Item(Col1DiscountPer, I).Value = Format(Math.Abs(AgL.VNull(.Rows(I)("DiscountPer"))), "0.00")
                    Dgl1.Item(Col1AdditionalDiscountPer, I).Value = Format(Math.Abs(AgL.VNull(.Rows(I)("AdditionalDiscountPer"))), "0.00")
                    Dgl1.Item(Col1AdditionPer, I).Value = Format(Math.Abs(AgL.VNull(.Rows(I)("AdditionPer"))), "0.00")
                Next I
            End If
        End With
        Calculation()
    ...
    Public Sub FSave(DocId As String, ByVal Conn As Object, ByVal Cmd As Object)
        ...
        For I = 0 To Dgl1.RowCount - 1
            If Dgl1.Item(Col1ItemCategory, I).Value <> Dgl1.Item(Col1xItemCategory, I).Value Or Dgl1.Item(Col1ItemGroup, I).Value <> Dgl1.Item(Col1xItemGroup, I).Value Or Not Dgl1.Rows(I).Visible Then
                If Dgl1.Item(Col1xItemCategory, I).Value <> "" And Dgl1.Item(Col1xItemGroup, I).Value <> "" Then
                    mQry = " Update ItemGroupPerson Set DiscountCalculationPattern = Null, DiscountPer=0, AdditionalDiscountPer=0 , AdditionPer=0 
        ...
        For I = 0 To Dgl1.RowCount - 1
            If Dgl1.Rows(I).Visible Then
                If (Dgl1.Item(Col1ItemGroup, I).Value <> "" Or Dgl1.Item(Col1ItemCategory, I).Value <> "") Then
                    If Dgl1.Item(Col1ItemCategory, I).Value <> "" And Dgl1.Item(Col1ItemGroup, I).Value <> "" Then
                        ...
                        If AgL.Dman_Execute(mQry, IIf(AgL.PubServerName = "", Conn, AgL.GcnRead)).ExecuteScalar() > 0 Then
                            ...
                            AgL.Dman_ExecuteNonQry(mQry, Conn, Cmd)
                        Else
                            mQry = " INSERT INTO ItemGroupPerson (ItemCategory, ItemGroup, Person, DiscountCalculationPattern, DiscountPer, AdditionalDiscountPer, AdditionPer) " &
        ...
        For I = 0 To Dgl1.RowCount - 1
            If Dgl1.Rows(I).Visible Then
                If Dgl1.Item(Col1ItemGroup, I).Value <> "" Then
                    ...
                    End If
                Else
                    mQry = "Update ItemGroupPerson set DiscountCalculationPattern=Null, DiscountPer=0, AdditionalDiscountPer=0, AdditionPer=0 Where ItemGroup = '" & Dgl1.Item(Col1ItemGroup, I).Tag & "' And Person = '" & DocId & "'  "
                    AgL.Dman_ExecuteNonQry(mQry, Conn, Cmd)
                End If
            End If
        Next
