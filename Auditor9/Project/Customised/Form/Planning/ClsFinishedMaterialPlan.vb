...
Public Class ClsFinishedMaterialPlan
    ...
    Public Const Col1SaleOrderNo As String = "Sale Order No"
    ...
    Public Const Col1PendingOrderQtyToPlan As String = "Pending Order Qty To Plan"
    ...
    Public Sub ProcSaleOrderPlan(Optional mFilterGrid As AgControls.AgDataGrid = Nothing,
        ...
        Try
            ...
            mQry = " SELECT 'o' As Tick, VSaleOrder.SaleOrder, VSaleOrder.SaleOrderSr, VSaleOrder.Party, 
                    ...
                    IsNull(VSaleOrder.SaleOrderQty,0) - IsNull(VPurchPlanDetailBase.PlanQty,0) AS PendingOrderQtyToPlan,
            ...
            ReportFrm.InputColumnsStr = "|" + Col1Qty + "|"
            ...
            ReportFrm.DGL1.Columns(Col1PendingOrderQtyToPlan).Visible = False
        ...
        If mFormat = mFormat_SaleOrderPendingForPlan Then
            For I As Integer = 0 To ReportFrm.DGL1.Rows.Count - 1
                If ReportFrm.DGL1.Item(Col1Select, I).Value = "þ" Then
                    If ReportFrm.DGL1.Item(Col1Qty, I).Value > ReportFrm.DGL1.Item(Col1PendingOrderQtyToPlan, I).Value Then
                        MsgBox("Plan Qty is exceeding pending order qty at row number " & (I + 1).ToString, MsgBoxStyle.Information)
                        ReportFrm.DGL1.CurrentCell = ReportFrm.DGL1.Item(Col1Qty, I)
            ...
            For I As Integer = 0 To ReportFrm.DGL1.Rows.Count - 1
                If ReportFrm.DGL1.Item(Col1Select, I).Value = "þ" Then
                    mQry = " INSERT INTO [" & bTempTable & "](SaleOrder, SaleOrderSr, SkuCode,
                        ...
                        Select " & AgL.Chk_Text(AgL.XNull(ReportFrm.DGL1.Item(Col1SaleOrder, I).Value)) & " As SaleOrder, 
                        " & Val(AgL.VNull(ReportFrm.DGL1.Item(Col1SaleOrderSr, I).Value)) & " As SaleOrderSr,
                        " & AgL.Chk_Text(AgL.XNull(ReportFrm.DGL1.Item(Col1SKUCode, I).Value)) & " As SKUCode,
            ...
            If AgL.VNull(AgL.Dman_Execute("Select Count(Distinct SaleOrder) From [" & bTempTable & "] As T With (NoLock) ", IIf(AgL.PubServerName = "", AgL.GCn, AgL.GcnRead)).ExecuteScalar()) > 1 Then
                MsgBox("Please select single sale order...!", MsgBoxStyle.Information)
                Exit Sub
            Else
                ReportFrm.FilterGrid.Item(GFilterCode, 5).Value = "'" + ReportFrm.DGL1.Item(Col1SaleOrder, 0).Value + "'"
                ReportFrm.FilterGrid.Item(GFilter, 5).Value = ReportFrm.DGL1.Item(Col1SaleOrderNo, 0).Value

                ReportFrm.FilterGrid.Item(GFilter, 6).Value = ReportFrm.DGL1.Item(Col1SaleOrderNo, 0).Value
            End If
        ...
        For I = 0 To ReportFrm.DGL1.RowCount - 1
            If AgL.XNull(ReportFrm.DGL1.Item(Col1SKU, I).Value) <> "" Then
                If Val(ReportFrm.DGL1.Item(bQtyColumnName, I).Value) > 0 Then
                    ...
                    mQry = "Select * From PurchPlanDetail L With (NoLock) Where L.DocId = " & AgL.Chk_Text(mPurchPlanDocId) & " And Sr = " & mSr & ""
                    Dim DtTemp1 As DataTable = AgL.FillData(mQry, IIf(AgL.PubServerName = "", Conn, AgL.GcnRead)).Tables(0)

                    mQry = "Select * From [" & bTempTable & "] L With (NoLock) "
                    Dim DtTemp2 As DataTable = AgL.FillData(mQry, IIf(AgL.PubServerName = "", Conn, AgL.GcnRead)).Tables(0)
                    ...
                    For J = 0 To bPurchPlanDetailBase.Rows.Count - 1
                        ...
                        mQry = "INSERT INTO PurchPlanDetailBaseSaleOrder (GenDocID, GenSr, 
                                ...
                                " & Val(mSr) & " As GenSr, 
                                " & AgL.Chk_Text(AgL.XNull(bPurchPlanDetailBase.Rows(J)("DocId"))) & " As PurchPlanDetailBase, 
                                " & Val(AgL.VNull(bPurchPlanDetailBase.Rows(J)("TSr"))) & " As PurchPlanDetailBaseTSr, 
                                " & Val(bPurchPlanDetailBaseSr) & " As PurchPlanDetailBaseSr, 
                                " & AgL.Chk_Text(AgL.XNull(bPurchPlanDetailBase.Rows(J)("SaleOrder"))) & " As SaleInvoice, 
                                " & Val(AgL.VNull(bPurchPlanDetailBase.Rows(J)("SaleOrderSr"))) & " As SaleInvoiceSr, 
                                " & Val(AgL.VNull(bPurchPlanDetailBase.Rows(J)("SaleOrderQty"))) & " As Qty "
                        AgL.Dman_ExecuteNonQry(mQry, Conn, Cmd)
                    Next
                End If
            End If
