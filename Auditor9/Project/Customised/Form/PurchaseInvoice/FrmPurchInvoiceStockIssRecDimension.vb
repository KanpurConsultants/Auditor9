...
Public Class FrmPurchaseInvoiceStockIssRecDimension
    Dim mQry As String = ""
    ...
    Dim mObjLineGrid As AgControls.AgDataGrid
    Dim mObjFrmPurchaseInvoiceStockIssRec As FrmPurchaseInvoiceStockIssRec
    ...
    Public Property DglRow() As DataGridViewRow
        ...
        End Set
    End Property
    Public Property objFrmPurchaseInvoiceStockIssRec() As FrmPurchaseInvoiceStockIssRec
        Get
            objFrmPurchaseInvoiceStockIssRec = mObjFrmPurchaseInvoiceStockIssRec
        End Get
        Set(ByVal value As FrmPurchaseInvoiceStockIssRec)
            mObjFrmPurchaseInvoiceStockIssRec = value
        End Set
    End Property
    Public Property objFrmPurchInvoice() As FrmPurchInvoiceDirect_WithDimension
        ...
        End Set
    End Property
    Public Property objLineGrid() As AgControls.AgDataGrid
        Get
            objLineGrid = mObjLineGrid
        End Get
    ...
    Private Sub DGL1_RowsAdded(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewRowsAddedEventArgs) Handles Dgl1.RowsAdded, Dgl1.RowsAdded
        ...
        Try
            If Dgl1.Columns.Count > 1 Then
                Dgl1.Item(Col1ItemType, e.RowIndex).Tag = AgL.XNull(DglRow.Cells(FrmPurchaseInvoiceStockIssRec.ColItemType).Tag)
                Dgl1.Item(Col1ItemCategory, e.RowIndex).Tag = AgL.XNull(DglRow.Cells(FrmPurchaseInvoiceStockIssRec.ColItemCategory).Tag)
                Dgl1.Item(Col1ItemGroup, e.RowIndex).Tag = AgL.XNull(DglRow.Cells(FrmPurchaseInvoiceStockIssRec.ColItemGroup).Tag)
                Dgl1.Item(Col1Item, e.RowIndex).Tag = AgL.XNull(DglRow.Cells(FrmPurchaseInvoiceStockIssRec.ColItem).Tag)
                Dgl1.Item(Col1Dimension1, e.RowIndex).Tag = AgL.XNull(DglRow.Cells(FrmPurchaseInvoiceStockIssRec.ColDimension1).Tag)
                Dgl1.Item(Col1Dimension2, e.RowIndex).Tag = AgL.XNull(DglRow.Cells(FrmPurchaseInvoiceStockIssRec.ColDimension2).Tag)
                Dgl1.Item(Col1Dimension3, e.RowIndex).Tag = AgL.XNull(DglRow.Cells(FrmPurchaseInvoiceStockIssRec.ColDimension3).Tag)
                Dgl1.Item(Col1Dimension4, e.RowIndex).Tag = AgL.XNull(DglRow.Cells(FrmPurchaseInvoiceStockIssRec.ColDimension4).Tag)
                Dgl1.Item(Col1Size, e.RowIndex).Tag = AgL.XNull(DglRow.Cells(FrmPurchaseInvoiceStockIssRec.ColSize).Tag)

                Dgl1.Item(Col1ItemCategory, e.RowIndex).Value = AgL.XNull(DglRow.Cells(FrmPurchaseInvoiceStockIssRec.ColItemCategory).Value)
                Dgl1.Item(Col1ItemGroup, e.RowIndex).Value = AgL.XNull(DglRow.Cells(FrmPurchaseInvoiceStockIssRec.ColItemGroup).Value)
                Dgl1.Item(Col1Item, e.RowIndex).Value = AgL.XNull(DglRow.Cells(FrmPurchaseInvoiceStockIssRec.ColItem).Value)
                Dgl1.Item(Col1Dimension1, e.RowIndex).Value = AgL.XNull(DglRow.Cells(FrmPurchaseInvoiceStockIssRec.ColDimension1).Value)
                Dgl1.Item(Col1Dimension2, e.RowIndex).Value = AgL.XNull(DglRow.Cells(FrmPurchaseInvoiceStockIssRec.ColDimension2).Value)
                Dgl1.Item(Col1Dimension3, e.RowIndex).Value = AgL.XNull(DglRow.Cells(FrmPurchaseInvoiceStockIssRec.ColDimension3).Value)
                Dgl1.Item(Col1Dimension4, e.RowIndex).Value = AgL.XNull(DglRow.Cells(FrmPurchaseInvoiceStockIssRec.ColDimension4).Value)
                Dgl1.Item(Col1Size, e.RowIndex).Value = AgL.XNull(DglRow.Cells(FrmPurchaseInvoiceStockIssRec.ColSize).Value)
            End If
        Catch ex As Exception
        End Try
    End Sub
            ...
            If Dgl1.Rows(I).Visible Then
                If Val(Dgl1.Item(Col1TotalQty, I).Value) <> 0 Then
                    If Dgl1.Item(ColSNo, I).Tag Is Nothing Then
                        ...
                        If mEntryNCatNature = NCatNature.Receive Then
                            bQty_Issue = 0
                            bQty_Receive = Val(Dgl1(Col1TotalQty, I).Value)
                        Else
                            bQty_Issue = Val(Dgl1(Col1TotalQty, I).Value)
                            bQty_Receive = 0
                        End If

                        If CType(AgL.VNull(mObjFrmPurchaseInvoiceStockIssRec.FGetSettings(ClsMain.SettingFields.PostInStockYn, SettingType.General)), Boolean) = True Then
                            mQry = "Insert Into Stock(DocID, TSr, Sr, V_Type, 
                                V_Prefix, V_Date, V_No, RecID, 
                                Div_Code, Site_Code, Godown, SubCode, Item, 
                                EType_IR, Qty_Rec, Qty_Iss, 
                                Unit, Rate, Amount, Landed_Value) 
                                Values
                                (
                                '" & DocId & "', " & TSr & ", " & mObjFrmPurchaseInvoiceStockIssRec.mDimensionSrl & ", 
                                " & AgL.Chk_Text(mObjFrmPurchInvoice.DglMain.Item(FrmPurchInvoiceDirect_WithDimension.Col1Value, objFrmPurchInvoice.rowV_Type).Tag) & ",
                                " & AgL.Chk_Text(objFrmPurchInvoice.LblPrefix.Text) & ", 
                                " & AgL.Chk_Date(mObjFrmPurchInvoice.DglMain.Item(FrmPurchInvoiceDirect_WithDimension.Col1Value, objFrmPurchInvoice.rowV_Date).Value) & ", 
                                " & AgL.Chk_Text(mObjFrmPurchInvoice.DglMain.Item(FrmPurchInvoiceDirect_WithDimension.Col1Value, objFrmPurchInvoice.rowV_No).Value) & ", 
                                " & AgL.Chk_Text(mObjFrmPurchInvoice.DglMain.Item(FrmPurchInvoiceDirect_WithDimension.Col1Value, objFrmPurchInvoice.rowReferenceNo).Value) & ",
                                " & AgL.Chk_Text(objFrmPurchInvoice.TxtDivision.Tag) & ", 
                                " & AgL.Chk_Text(mObjFrmPurchInvoice.DglMain.Item(FrmPurchInvoiceDirect_WithDimension.Col1Value, objFrmPurchInvoice.rowSite_Code).Tag) & ", 
                                " & AgL.Chk_Text(mObjFrmPurchaseInvoiceStockIssRec.DglMain.Item(FrmPurchaseInvoiceStockIssRec.Col1Value, mObjFrmPurchaseInvoiceStockIssRec.rowGodown).Tag) & ", 
                                " & AgL.Chk_Text(mObjFrmPurchInvoice.DglMain.Item(FrmPurchInvoiceDirect_WithDimension.Col1Value, objFrmPurchInvoice.rowVendor).Tag) & ", 
                                " & AgL.Chk_Text(Dgl1(Col1SKU, I).Tag) & ", 
                                'R', 
                                " & bQty_Receive & ", " & bQty_Issue & ",
                                " & AgL.Chk_Text(mObjLineGrid(FrmPurchaseInvoiceStockIssRec.ColUnit, mGridRowIndex).Value) & ", 
                                " & AgL.Chk_Text(mObjLineGrid(FrmPurchaseInvoiceStockIssRec.ColRate, mGridRowIndex).Value) & ", 
                                " & AgL.Chk_Text(mObjLineGrid(FrmPurchaseInvoiceStockIssRec.ColAmount, mGridRowIndex).Value) & ",0
                            )   "
                            AgL.Dman_ExecuteNonQry(mQry, Conn, Cmd)
                        End If

                        If CType(AgL.VNull(mObjFrmPurchaseInvoiceStockIssRec.FGetSettings(ClsMain.SettingFields.PostInStockProcessYn, SettingType.General)), Boolean) = True Then
                        ...
                        If mDimensionDetail <> "" Then
                            ...
                            Else
                                mDimensionDetail += Dgl1.Item(Col1Qty, I).Value.ToString + " X " + Dgl1.Item(Col1Pcs, I).Value.ToString
                            End If
                        Else
                            If Dgl1.Item(Col1Dimension1, I).Value <> "" And Dgl1.Columns(Col1Dimension1).Visible = True Then mDimensionDetail += Dgl1.Item(Col1Dimension1, I).Value + "-"
                            If Dgl1.Item(Col1Dimension2, I).Value <> "" And Dgl1.Columns(Col1Dimension2).Visible = True Then mDimensionDetail += Dgl1.Item(Col1Dimension2, I).Value + "-"
                            ...
                            If Dgl1.Item(Col1Dimension4, I).Value <> "" And Dgl1.Columns(Col1Dimension4).Visible = True Then mDimensionDetail += Dgl1.Item(Col1Dimension4, I).Value + "-"
                            If Dgl1.Columns(Col1Pcs).DisplayIndex < Dgl1.Columns(Col1Qty).DisplayIndex Then
                                mDimensionDetail += AgL.VNull(Dgl1.Item(Col1Pcs, I).Value).ToString + " X " + AgL.VNull(Dgl1.Item(Col1Qty, I).Value).ToString
                            Else
                                mDimensionDetail += AgL.VNull(Dgl1.Item(Col1Qty, I).Value).ToString + " X " + AgL.VNull(Dgl1.Item(Col1Pcs, I).Value).ToString
                            End If
                        End If
                    ...
                    Else
                        ...
                        If CType(AgL.VNull(mObjFrmPurchaseInvoiceStockIssRec.FGetSettings(ClsMain.SettingFields.PostInStockYn, SettingType.General)), Boolean) = True Then
                            If AgL.VNull(AgL.Dman_Execute(" Select Count(*) From Stock With (NoLock)
                                ...
                                mQry = "Update Stock Set V_Type = " & AgL.Chk_Text(mObjFrmPurchInvoice.DglMain.Item(FrmPurchInvoiceDirect_WithDimension.Col1Value, objFrmPurchInvoice.rowV_Type).Tag) & ",
                                    V_Prefix = " & AgL.Chk_Text(objFrmPurchInvoice.LblPrefix.Text) & ", 
                                    V_Date = " & AgL.Chk_Date(mObjFrmPurchInvoice.DglMain.Item(FrmPurchInvoiceDirect_WithDimension.Col1Value, objFrmPurchInvoice.rowV_Date).Value) & ", 
                                    V_No = " & AgL.Chk_Text(mObjFrmPurchInvoice.DglMain.Item(FrmPurchInvoiceDirect_WithDimension.Col1Value, objFrmPurchInvoice.rowV_No).Value) & ", 
                                    RecId = " & AgL.Chk_Text(mObjFrmPurchInvoice.DglMain.Item(FrmPurchInvoiceDirect_WithDimension.Col1Value, objFrmPurchInvoice.rowReferenceNo).Value) & ",
                                    Div_Code = " & AgL.Chk_Text(objFrmPurchInvoice.TxtDivision.Tag) & ", 
                                    Site_Code = " & AgL.Chk_Text(mObjFrmPurchInvoice.DglMain.Item(FrmPurchInvoiceDirect_WithDimension.Col1Value, objFrmPurchInvoice.rowSite_Code).Tag) & ", 
                                    Godown = " & AgL.Chk_Text(mObjFrmPurchaseInvoiceStockIssRec.DglMain.Item(FrmPurchaseInvoiceStockIssRec.Col1Value, mObjFrmPurchaseInvoiceStockIssRec.rowGodown).Tag) & ", 
                                    Subcode = " & AgL.Chk_Text(mObjFrmPurchInvoice.DglMain.Item(FrmPurchInvoiceDirect_WithDimension.Col1Value, objFrmPurchInvoice.rowVendor).Tag) & ", 
                                    Item = " & AgL.Chk_Text(Dgl1(Col1SKU, I).Tag) & ",
                                    EType_IR = 'R', 
                                    Qty_Rec = " & bQty_Receive & ", 
                                    Qty_Iss = " & bQty_Issue & ",
                                    Unit=" & AgL.Chk_Text(mObjLineGrid(FrmPurchaseInvoiceStockIssRec.ColUnit, mGridRowIndex).Value) & ", 
                                    Rate=" & AgL.Chk_Text(mObjLineGrid(FrmPurchaseInvoiceStockIssRec.ColRate, mGridRowIndex).Value) & ", 
                                    Amount=" & AgL.Chk_Text(mObjLineGrid(FrmPurchaseInvoiceStockIssRec.ColAmount, mGridRowIndex).Value) & ",
                                    Landed_Value=0                                
                                    Where DocID = '" & DocId & "' And TSr = " & TSr & " And Sr = " & Dgl1.Item(ColSNo, I).Tag & "
                                "
                                AgL.Dman_ExecuteNonQry(mQry, Conn, Cmd)
                            Else
                                ...
                                mQry = "Insert Into Stock(DocID, TSr, Sr, V_Type, V_Prefix, V_Date, V_No, RecId,
                                    Div_Code, Site_Code, Godown, Subcode, Item, EType_IR, Qty_Rec, Qty_Iss, 
                                    Unit, Rate, Amount, Landed_Value)
                                    Values('" & DocId & "', " & TSr & ", " & mSr & ",
                                    " & AgL.Chk_Text(mObjFrmPurchInvoice.DglMain.Item(FrmPurchInvoiceDirect_WithDimension.Col1Value, objFrmPurchInvoice.rowV_Type).Tag) & ",
                                    " & AgL.Chk_Text(objFrmPurchInvoice.LblPrefix.Text) & ", 
                                    " & AgL.Chk_Date(mObjFrmPurchInvoice.DglMain.Item(FrmPurchInvoiceDirect_WithDimension.Col1Value, objFrmPurchInvoice.rowV_Date).Value) & ", 
                                    " & AgL.Chk_Text(mObjFrmPurchInvoice.DglMain.Item(FrmPurchInvoiceDirect_WithDimension.Col1Value, objFrmPurchInvoice.rowV_No).Value) & ", 
                                    ...
                                    " & AgL.Chk_Text(mObjLineGrid(FrmPurchaseInvoiceStockIssRec.ColUnit, mGridRowIndex).Value) & ", 
                                    " & AgL.Chk_Text(mObjLineGrid(FrmPurchaseInvoiceStockIssRec.ColRate, mGridRowIndex).Value) & ", 
                                    " & AgL.Chk_Text(mObjLineGrid(FrmPurchaseInvoiceStockIssRec.ColAmount, mGridRowIndex).Value) & ",
                                    0) "
                                AgL.Dman_ExecuteNonQry(mQry, Conn, Cmd)
                            End If
                        End If
                        ...
                        If CType(AgL.VNull(mObjFrmPurchaseInvoiceStockIssRec.FGetSettings(ClsMain.SettingFields.PostInStockProcessYn, SettingType.General)), Boolean) = True Then
                            If AgL.VNull(AgL.Dman_Execute(" Select Count(*) From StockProcess With (NoLock)
                                        Where DocId = '" & DocId & "' And TSr = " & TSr & " 
                                        And Sr = " & Dgl1.Item(ColSNo, I).Tag & "", IIf(AgL.PubServerName = "", Conn, AgL.GcnRead)).ExecuteScalar()) > 0 Then
                                mQry = "Update StockProcess Set V_Type = " & AgL.Chk_Text(mObjFrmPurchInvoice.DglMain.Item(FrmPurchInvoiceDirect_WithDimension.Col1Value, objFrmPurchInvoice.rowV_Type).Tag) & ",
                                    V_Prefix = " & AgL.Chk_Text(objFrmPurchInvoice.LblPrefix.Text) & ", 
                                    V_Date = " & AgL.Chk_Date(mObjFrmPurchInvoice.DglMain.Item(FrmPurchInvoiceDirect_WithDimension.Col1Value, objFrmPurchInvoice.rowV_Date).Value) & ", 
                                    V_No = " & AgL.Chk_Text(mObjFrmPurchInvoice.DglMain.Item(FrmPurchInvoiceDirect_WithDimension.Col1Value, objFrmPurchInvoice.rowV_No).Value) & ", 
                                    RecId = " & AgL.Chk_Text(mObjFrmPurchInvoice.DglMain.Item(FrmPurchInvoiceDirect_WithDimension.Col1Value, objFrmPurchInvoice.rowReferenceNo).Value) & ",
                            ...
                            Else
                                mSr = AgL.VNull(AgL.Dman_Execute(" Select IfNull(Max(Sr),0) + 1 From StockProcess With (NoLock)
                                        Where DocId = '" & DocId & "' And TSr = " & TSr & "", IIf(AgL.PubServerName = "", Conn, AgL.GcnRead)).ExecuteScalar())
                                mQry = "Insert Into StockProcess(DocID, TSr, Sr, V_Type, V_Prefix, V_Date, V_No, RecId,
                                    Div_Code, Site_Code, Godown, Subcode, Item, EType_IR, Qty_Rec, Qty_Iss, 
                                    Unit, Rate, Amount, Landed_Value)
                                    Values('" & DocId & "', " & TSr & ", " & mSr & ",
                                    " & AgL.Chk_Text(mObjFrmPurchInvoice.DglMain.Item(FrmPurchInvoiceDirect_WithDimension.Col1Value, objFrmPurchInvoice.rowV_Type).Tag) & ",
            ...
            For I As Integer = 0 To Dgl1.Rows.Count - 1
                If AgL.VNull(Dgl1.Item(Col1Qty, I).Value) > 0 Then
                    If Dgl1.Columns(Col1ItemType).Visible = False Then
                        Dgl1.Item(Col1ItemType, I).Tag = AgL.XNull(DglRow.Cells(FrmPurchaseInvoiceStockIssRec.ColItemType).Tag)
                        Dgl1.Item(Col1ItemType, I).Value = AgL.XNull(DglRow.Cells(FrmPurchaseInvoiceStockIssRec.ColItemType).Value)
                    End If
                    If Dgl1.Columns(Col1ItemCategory).Visible = False Then
                        Dgl1.Item(Col1ItemCategory, I).Tag = AgL.XNull(DglRow.Cells(FrmPurchaseInvoiceStockIssRec.ColItemCategory).Tag)
                        Dgl1.Item(Col1ItemCategory, I).Value = AgL.XNull(DglRow.Cells(FrmPurchaseInvoiceStockIssRec.ColItemCategory).Value)
                    End If
                    If Dgl1.Columns(Col1ItemGroup).Visible = False Then
                        Dgl1.Item(Col1ItemGroup, I).Tag = AgL.XNull(DglRow.Cells(FrmPurchaseInvoiceStockIssRec.ColItemGroup).Tag)
                        Dgl1.Item(Col1ItemGroup, I).Value = AgL.XNull(DglRow.Cells(FrmPurchaseInvoiceStockIssRec.ColItemGroup).Value)
                    End If
                    If Dgl1.Columns(Col1Item).Visible = False Then
                        Dgl1.Item(Col1Item, I).Tag = AgL.XNull(DglRow.Cells(FrmPurchaseInvoiceStockIssRec.ColItem).Tag)
                        Dgl1.Item(Col1Item, I).Value = AgL.XNull(DglRow.Cells(FrmPurchaseInvoiceStockIssRec.ColItem).Value)
                    End If
                    If Dgl1.Columns(Col1Dimension1).Visible = False Then
                        Dgl1.Item(Col1Dimension1, I).Tag = AgL.XNull(DglRow.Cells(FrmPurchaseInvoiceStockIssRec.ColDimension1).Tag)
                        Dgl1.Item(Col1Dimension1, I).Value = AgL.XNull(DglRow.Cells(FrmPurchaseInvoiceStockIssRec.ColDimension1).Value)
                    End If
                    If Dgl1.Columns(Col1Dimension2).Visible = False Then
                        Dgl1.Item(Col1Dimension2, I).Tag = AgL.XNull(DglRow.Cells(FrmPurchaseInvoiceStockIssRec.ColDimension2).Tag)
                        Dgl1.Item(Col1Dimension2, I).Value = AgL.XNull(DglRow.Cells(FrmPurchaseInvoiceStockIssRec.ColDimension2).Value)
                    End If
                    If Dgl1.Columns(Col1Dimension3).Visible = False Then
                        Dgl1.Item(Col1Dimension3, I).Tag = AgL.XNull(DglRow.Cells(FrmPurchaseInvoiceStockIssRec.ColDimension3).Tag)
                        Dgl1.Item(Col1Dimension3, I).Value = AgL.XNull(DglRow.Cells(FrmPurchaseInvoiceStockIssRec.ColDimension3).Value)
                    End If
                    If Dgl1.Columns(Col1Dimension4).Visible = False Then
                        Dgl1.Item(Col1Dimension4, I).Tag = AgL.XNull(DglRow.Cells(FrmPurchaseInvoiceStockIssRec.ColDimension4).Tag)
                        Dgl1.Item(Col1Dimension4, I).Value = AgL.XNull(DglRow.Cells(FrmPurchaseInvoiceStockIssRec.ColDimension4).Value)
                    End If
                    If Dgl1.Columns(Col1Size).Visible = False Then
                        Dgl1.Item(Col1Size, I).Tag = AgL.XNull(DglRow.Cells(FrmPurchaseInvoiceStockIssRec.ColSize).Tag)
                        Dgl1.Item(Col1Size, I).Value = AgL.XNull(DglRow.Cells(FrmPurchaseInvoiceStockIssRec.ColSize).Value)
                    End If
                End If
            Next
        Catch ex As Exception
