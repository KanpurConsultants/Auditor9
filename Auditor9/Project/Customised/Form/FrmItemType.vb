...
#End Region
    ...
    Private Sub FrmYarn_BaseEvent_Data_Validation(ByRef passed As Boolean) Handles Me.BaseEvent_Data_Validation
        ...
        If Topctrl1.Mode = "Add" Then
            mQry = "Select count(*) From ItemType Where Code='" & DglMain.Item(Col1Value, rowCode).Value & "'  "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then Err.Raise(1, , "Code Already Exist!")

            mQry = "Select count(*) From ItemType Where Name='" & DglMain.Item(Col1Value, rowName).Value & "'  "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then Err.Raise(1, , "Name Already Exist!")
        Else
            mQry = "Select count(*) From ItemType Where Code='" & DglMain.Item(Col1Value, rowCode).Value & "' And Code<>'" & mInternalCode & "' "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then Err.Raise(1, , "Code Already Exist!")

            mQry = "Select count(*) From ItemType Where Name='" & DglMain.Item(Col1Value, rowName).Value & "' And Code<>'" & mInternalCode & "' "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then Err.Raise(1, , "Name Already Exist!")
        End If
    ...
    Private Sub FrmQuality1_BaseFunction_MoveRec(ByVal SearchCode As String) Handles Me.BaseFunction_MoveRec
        ...
        mQry = "Select H.*, P.Name As ParentName  " &
            " From ItemType H " &
            " LEFT JOIN ItemType P ON H.Parent = P.Code " &
    ...
    Private Function FGetRelationalData() As Boolean
        Try
            'mQry = " Select Count(*) From ItemType Where Code = '" & mSearchCode & "'"
