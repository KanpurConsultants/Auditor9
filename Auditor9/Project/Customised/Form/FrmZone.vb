...
#Region "Designer Code"
    Private Sub InitializeComponent()
        ...
        'GBoxEntryType
        '
        Me.GBoxEntryType.Location = New System.Drawing.Point(148, 286)
        '
        'TxtEntryType
        '
        Me.TxtEntryType.Tag = ""
        ...
        'GBoxMoveToLog
        '
        Me.GBoxMoveToLog.Location = New System.Drawing.Point(221, 223)
        '
        'TxtMoveToLog
        '
        Me.TxtMoveToLog.Tag = ""
        ...
        'GBoxDivision
        '
        Me.GBoxDivision.Location = New System.Drawing.Point(457, 223)
...
#End Region
    ...
    Private Sub FrmYarn_BaseEvent_Data_Validation(ByRef passed As Boolean) Handles Me.BaseEvent_Data_Validation
        ...
        If Topctrl1.Mode = "Add" Then
            mQry = "Select count(*) From Zone Where Description='" & TxtDescription.Text & "' "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then Err.Raise(1, , "Description Already Exist!")
        Else
            mQry = "Select count(*) From Zone Where Description='" & TxtDescription.Text & "' And Code <> '" & mInternalCode & "' "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then Err.Raise(1, , "Description Already Exist!")
        End If
    End Sub
    ...
    Public Overridable Sub FrmYarn_BaseEvent_FindMain() Handles Me.BaseEvent_FindMain
        ...
        AgL.PubFindQry = "SELECT I.Code As SearchCode, I.Description  FROM Zone I "
    ...
    Private Sub FrmZone_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    ...
    Private Function FGetRelationalData() As Boolean
        Try
            'mQry = " Select Count(*) From ItemCategory Where Zone = '" & mSearchCode & "'"
    ...
    Private Sub FrmZone_BaseFunction_BlankText() Handles Me.BaseFunction_BlankText
        Dim obj As Object
    ...
    Public Shared Sub ImportZoneTable(ZoneTable As StructZone)
        Dim mQry As String = ""
        If AgL.Dman_Execute("SELECT Count(*) From Zone With (NoLock) Where Description = '" & ZoneTable.Description & "'", AgL.GCn).ExecuteScalar = 0 Then
            mQry = " INSERT INTO Zone(Code, Description, EntryBy, EntryDate, EntryType, EntryStatus, OMSId)
                    Select '" & ZoneTable.Code & "' As ZoneCode, " & AgL.Chk_Text(ZoneTable.Description) & " As Zone, 
                    '" & ZoneTable.EntryBy & "' As EntryBy, " & AgL.Chk_Date(ZoneTable.EntryDate) & " As EntryDate, 
                    '" & ZoneTable.EntryType & "' As EntryType, '" & ZoneTable.EntryStatus & "' As EntryStatus, 
                    '" & ZoneTable.OMSId & "' As OMSId "
            AgL.Dman_ExecuteNonQry(mQry, AgL.GCn, AgL.ECmd)
        Else
            mQry = " UPDATE Zone Set OMSId = '" & ZoneTable.OMSId & "' 
                    Where Description = '" & ZoneTable.Description & "'"
            AgL.Dman_ExecuteNonQry(mQry, AgL.GCn, AgL.ECmd)
        End If
    End Sub
    Public Structure StructZone
        Dim Code As String
