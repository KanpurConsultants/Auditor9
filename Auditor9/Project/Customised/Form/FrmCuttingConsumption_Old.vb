...
Imports System.Linq
Public Class FrmCuttingConsumption_Old
    ...
    Public Const rowBatchQty As Integer = 1
    Public Const rowBtnFill As Integer = 2
    ...
    Public Const Col1MainItemSize As String = "Main Item Size"
    Public Const Col1Item As String = "Item"
    Public Const Col1BomItemWidth As String = "Bom Item Width"
...
#End Region
    Private Sub FrmShade_BaseEvent_FindMain() Handles Me.BaseEvent_FindMain
        ...
        AgL.PubFindQry = " Select T.DocId As SearchCode, Max(Ic.Description) As ItemCategory "
        AgL.PubFindQry += " From TransactionReferences T "
        AgL.PubFindQry += " LEFT JOIN ItemCategory Ic On T.DocId = Ic.Code "
        AgL.PubFindQry += " Where Type = '" & mTransactionReferenceType & "'"
        AgL.PubFindQry += " Group by DocId "
        AgL.PubFindQry += " Order by DocId "
    ...
    Private Sub FrmShade_BaseFunction_FIniMast(ByVal BytDel As Byte, ByVal BytRefresh As Byte) Handles Me.BaseFunction_FIniMast
        mQry = " Select DocId As SearchCode From TransactionReferences Where Type = '" & mTransactionReferenceType & "'"
        mQry += " Group by DocId "
        mQry += " Order by DocId "
    ...
    Private Sub FrmQuality1_BaseFunction_MoveRec(ByVal SearchCode As String) Handles Me.BaseFunction_MoveRec
        ...
        mQry = " SELECT Ic.Code, Ic.Description
                FROM TransactionReferences H 
                LEFT JOIN ItemCategory Ic ON H.DocID = Ic.Code
                WHERE H.DocID = '" & SearchCode & "'
                And H.Type = '" & mTransactionReferenceType & "'"

        mQry = "Select I.*  From Item I Where I.Code = '" & SearchCode & "'"
        DtTemp = AgL.FillData(mQry, AgL.GCn).Tables(0)

        With DtTemp
            If .Rows.Count > 0 Then
                mInternalCode = AgL.XNull(.Rows(0)("Code"))
                DglMain.Item(Col1Value, rowItemCategory).Tag = AgL.XNull(.Rows(0)("Code"))
                DglMain.Item(Col1Value, rowItemCategory).Value = AgL.XNull(.Rows(0)("Description"))
            End If
        End With

        mQry = "SELECT L.*, I.Description AS MainItemDesc, BomItem.Description AS BomItemDesc, 
                S.Code As MainItemSize, S.Description As MainItemSizeDesc, 
                Raw.Code As RawMaterialCode, Raw.Description As RawMaterialDesc, BomItem.Unit, 
                I.DealQty As BatchQty
                FROM TransactionReferences H 
                LEFT JOIN BomDetail L ON H.ReferenceDocId = L.Code
                LEFT JOIN Item I ON L.Code = I.Code
                LEFT JOIN Size S ON I.Size = S.Code
                LEFT JOIN Item Raw On I.RawMaterial = Raw.Code
        ...
        With DtTemp
            ...
            If .Rows.Count > 0 Then
                For I = 0 To DtTemp.Rows.Count - 1
                    ...
                    Dgl1.Item(Col1MainItemSize, I).Tag = AgL.XNull(.Rows(I)("MainItemSize"))
                    Dgl1.Item(Col1MainItemSize, I).Value = AgL.XNull(.Rows(I)("MainItemSizeDesc"))
                    Dgl1.Item(Col1Item, I).Tag = AgL.XNull(.Rows(I)("Item"))
                    Dgl1.Item(Col1Item, I).Value = AgL.XNull(.Rows(I)("BomItemDesc"))
                    Dgl1.Item(Col1BomItemWidth, I).Tag = AgL.XNull(.Rows(I)("RawMaterialCode"))
                    Dgl1.Item(Col1BomItemWidth, I).Value = AgL.XNull(.Rows(I)("RawMaterialDesc"))
    ...
    Private Sub FrmParty_BaseEvent_Topctrl_tbEdit(ByRef Passed As Boolean) Handles Me.BaseEvent_Topctrl_tbEdit
        ...
        DglMain.Focus()
    End Sub
    Private Sub FrmCuttingConsumption_BaseFunction_IniGrid() Handles Me.BaseFunction_IniGrid
        Dim I As Integer
        ...
        With AgCL
            ...
            .AddAgTextColumn(Dgl1, Col1MainItemSize, 100, 0, Col1MainItemSize, True, False)
            .AddAgTextColumn(Dgl1, Col1Item, 150, 0, Col1Item, True, False)
            .AddAgTextColumn(Dgl1, Col1BomItemWidth, 100, 0, Col1BomItemWidth, True, False)
    ...
    Private Sub FrmCuttingConsumption_BaseFunction_BlankText() Handles Me.BaseFunction_BlankText
        Dim I As Integer
        For I = 0 To DglMain.Rows.Count - 1
    ...
    Private Sub Dgl1_EditingControlShowing(sender As Object, e As DataGridViewEditingControlShowingEventArgs) Handles Dgl1.EditingControlShowing, DglMain.EditingControlShowing
        ...
        End If
    End Sub
    Private Sub FrmCuttingConsumption_BaseEvent_Data_Validation(ByRef passed As Boolean) Handles Me.BaseEvent_Data_Validation
        Dim I As Integer
    ...
    Private Sub DglMain_EditingControl_Validating(sender As Object, e As CancelEventArgs) Handles DglMain.EditingControl_Validating
                ...
                If DglMain.CurrentCell.ColumnIndex = DglMain.Columns(Col1Value).Index Then
                    Select Case DglMain.CurrentCell.RowIndex
                        Case rowItemCategory
                            ...
                            Next
                            Dgl1.AgHelpDataSet(Col1MainItemSize) = Nothing
        ...
        End Try
    End Sub
    Private Sub FrmCuttingConsumption_BaseEvent_Topctrl_tbRef() Handles Me.BaseEvent_Topctrl_tbRef
        Dim I As Integer
        For I = 0 To DglMain.Rows.Count - 1
        ...
        Dgl1.AgHelpDataSet(Col1BomItemWidth) = Nothing
    End Sub
    ...
    Private Sub Dgl1_EditingControl_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Dgl1.EditingControl_KeyDown
        Try
            Select Case Dgl1.Columns(Dgl1.CurrentCell.ColumnIndex).Name
                Case Col1MainItemSize
                    If e.KeyCode <> Keys.Enter Then
                        If Dgl1.AgHelpDataSet(Col1MainItemSize) Is Nothing Then
                            mQry = " Select H.Code, H.Description From Size H Where ItemCategory = '" & DglMain.Item(Col1Value, rowItemCategory).Tag & "' Order By H.Description "
                            Dgl1.AgHelpDataSet(Col1MainItemSize) = AgL.FillData(mQry, AgL.GCn)
                        End If
                    End If

                Case Col1BomItemWidth
                    If e.KeyCode <> Keys.Enter Then
                        If Dgl1.AgHelpDataSet(Col1BomItemWidth) Is Nothing Then
                            mQry = " Select H.Code, H.Description From Dimension4 H Order By H.Description "
                            Dgl1.AgHelpDataSet(Col1BomItemWidth) = AgL.FillData(mQry, AgL.GCn)
                        End If
                    End If
                ...
                If Dgl1.Item(Col1Item, I).Value <> "" Then
                    If AgL.XNull(Dgl1.Item(Col1GeneratedItem, I).Tag) = "" Then
                        ...
                        mQry = " INSERT INTO Item (Code, ManualCode, Description, Unit, EntryBy, EntryDate, Status, 
                             Div_Code, Specification, ItemCategory, DealQty, Size, RawMaterial, StockYN, V_Type)
                             ...
                             " & AgL.Chk_Text(Dgl1.Item(Col1MainItemSize, I).Tag) & " As Dimension1, 
                             " & AgL.Chk_Text(Dgl1.Item(Col1BomItemWidth, I).Tag) & " As RawMaterial, 
                        ...
                        mQry = "INSERT INTO TransactionReferences (DocID, ReferenceDocId, Type)
                            Select " & AgL.Chk_Text(DglMain.Item(Col1Value, rowItemCategory).Tag) & " As DocID,
                            " & AgL.Chk_Text(bGeneratedMainItemCode) & " As ReferenceDocId,
                            " & AgL.Chk_Text(mTransactionReferenceType) & " As Type "
                        AgL.Dman_ExecuteNonQry(mQry, AgL.GCn, AgL.ECmd)
                    Else
                        mQry = "UPDATE Item 
                            SET Description = " & AgL.Chk_Text(Dgl1.Item(Col1GeneratedItem, I).Value) & ", 
                            Size = " & AgL.Chk_Text(Dgl1.Item(Col1MainItemSize, I).Tag) & ", 
                            RawMaterial = " & AgL.Chk_Text(Dgl1.Item(Col1BomItemWidth, I).Tag) & ", 
    ...
    Function FCreateBomItemDesc(bRowIndex As Integer) As String
        ...
        bItemDesc += "-"
        bItemDesc += Dgl1.Item(Col1MainItemSize, bRowIndex).Value
        bItemDesc += "-"
        bItemDesc += Dgl1.Item(Col1BomItemWidth, bRowIndex).Value
    ...
    Private Sub Dgl1_EditingControl_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles Dgl1.EditingControl_Validating
        ...
        Try
            ...
            Select Case Dgl1.Columns(Dgl1.CurrentCell.ColumnIndex).Name
                Case Col1MainItemSize, Col1BomItemWidth
        ...
        End Try
    End Sub
    Private Sub FrmCuttingConsumption_BaseEvent_ApproveDeletion_InTrans(SearchCode As String, Conn As Object, Cmd As Object) Handles Me.BaseEvent_ApproveDeletion_InTrans
        mQry = " Select * From TransactionReferences With (NoLock) 
                Where DocId = '" & SearchCode & "' And Type = '" & mTransactionReferenceType & "'"
        Dim DtTemp As DataTable = AgL.FillData(mQry, IIf(AgL.PubServerName = "", AgL.GCn, AgL.GcnRead)).Tables(0)

        For I As Integer = 0 To DtTemp.Rows.Count - 1
            mQry = " Delete From BomDetail Where Code = '" & AgL.XNull(DtTemp.Rows(I)("ReferenceDocId")) & "'"
            AgL.Dman_ExecuteNonQry(mQry, AgL.GCn, AgL.ECmd)

            mQry = " Delete From Item Where Code = '" & AgL.XNull(DtTemp.Rows(I)("ReferenceDocId")) & "'"
            AgL.Dman_ExecuteNonQry(mQry, AgL.GCn, AgL.ECmd)
        Next

        mQry = " Delete From TransactionReferences Where DocId = '" & SearchCode & "' And Type = '" & mTransactionReferenceType & "'"
        AgL.Dman_ExecuteNonQry(mQry, AgL.GCn, AgL.ECmd)
    End Sub
    ...
    Private Sub DglMain_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles DglMain.CellContentClick
        ...
        End Try
    End Sub
    Private Sub FHPGD_FillSizes()
        Dim StrRtn As String = ""
        Dim mLineCond As String = ""

        mQry = "Select Code, Description From Item Where V_Type = '" & ItemV_Type.SIZE & "' "
        ...
        For I As Integer = 0 To DtTemp.Rows.Count - 1
            ...
            Dgl1.Item(Col1MainItemSize, I).Tag = AgL.XNull(DtTemp.Rows(I)("Code"))
            Dgl1.Item(Col1MainItemSize, I).Value = AgL.XNull(DtTemp.Rows(I)("Description"))
        Next
    End Sub
End Class
