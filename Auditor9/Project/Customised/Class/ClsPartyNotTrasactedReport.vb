...
Public Class ClsPartyNotTransactedReport
    ...
    Dim rowDivision As Integer = 1
    Dim rowPartyType As Integer = 2
    Dim rowMoreThanDays As Integer = 3
    ...
    Private Sub ObjRepFormGlobal_ProcessReport() Handles ReportFrm.ProcessReport
        ProcPartyNotTransactedReport()
    End Sub
    Public Sub New(ByVal mReportFrm As FrmRepDisplay)
        ReportFrm = mReportFrm
    End Sub
    Public Sub ProcPartyNotTransactedReport(Optional mFilterGrid As AgControls.AgDataGrid = Nothing,
        ...
        Try
            ...
            RepTitle = "Party Not Transacted Report"
            ...
            mCondStr = " Where H.DocId Is Not Null "
            ...
            Dim bHavingClause As String = ""
            Dim strDate As String = AgL.Chk_Text(CDate(AgL.PubLoginDate).ToString("s"))
            If AgL.PubServerName = "" Then
                bHavingClause += " Having julianday(" & strDate & ")  - julianday(Max(H.V_Date))  >= " & AgL.VNull(ReportFrm.FGetText(rowMoreThanDays)).ToString()
            Else
                bHavingClause += " Having DateDiff(Day,Max(H.V_Date), " & strDate & ") >= " & AgL.VNull(ReportFrm.FGetText(rowMoreThanDays)).ToString()
            End If

            mQry = "SELECT Max(Sg.SubgroupType) AS PartyType, Max(Sg.Name) AS PartyName, strftime('%d/%m/%Y',Max(H.V_Date)) AS LastBillDate, Max(VBalance.Balance) AS Balance
                    FROM ViewHelpSubgroup Sg 
                    LEFT JOIN SaleInvoice H ON Sg.Code = H.SaleToParty
                    LEFT JOIN (
	                    SELECT L.SubCode, IfNull(Sum(L.AmtDr),0) - IfNull(Sum(L.AmtCr),0) AS Balance
	                    FROM Ledger L 
	                    GROUP BY L.SubCode
                    ) AS VBalance ON Sg.Code = VBalance.SubCode " & mCondStr &
                    " GROUP BY Sg.Code " & bHavingClause & "

                    UNION ALL

                    SELECT Max(Sg.SubgroupType) AS PartyType, Max(Sg.Name) AS PartyName, strftime('%d/%m/%Y',Max(H.V_Date)) AS LastBillDate, Max(VBalance.Balance) AS Balance
                    FROM ViewHelpSubgroup Sg 
                    LEFT JOIN PurchInvoice H ON Sg.Code = H.Vendor
                    LEFT JOIN (
	                    SELECT L.SubCode, IfNull(Sum(L.AmtDr),0) - IfNull(Sum(L.AmtCr),0) AS Balance
	                    FROM Ledger L 
	                    GROUP BY L.SubCode
            ...
            ReportFrm.Text = "Party Not Transacted Report"
            ReportFrm.ClsRep = Me
            ReportFrm.ReportProcName = "ProcPartyNotTransactedReport"
