...
Public Class ClsCashCustomerReport
    ...
    Private Sub ObjRepFormGlobal_ProcessReport() Handles ReportFrm.ProcessReport
        ProcCashCustomerReport()
    End Sub
    Public Sub New(ByVal mReportFrm As FrmRepDisplay)
        ReportFrm = mReportFrm
    End Sub
    Public Sub ProcCashCustomerReport(Optional mFilterGrid As AgControls.AgDataGrid = Nothing,
        ...
        Try
            ...
            mQry = "SELECT H.DocId, H.ManualRefNo, strftime('%d/%m/%Y', H.V_Date) As V_Date, H.V_Date As V_Date_ActualFormat, 
                    H.SaleToPartyName, H.SaleToPartyMobile, H.Net_Amount
                    FROM SaleInvoice H
                    LEFT JOIN Subgroup Sg ON H.BillToParty = Sg.Subcode
                    WHERE (Sg.Nature = 'Cash' Or Sg.SubGroupType = '" & SubgroupType.RevenuePoint & "')
                    AND IsNull(H.SaleToPartyMobile,'') <> '' " & mCondStr

            If ReportFrm.FGetText(rowReportType) = "Detail" Then
                mQry = " Select VMain.DocId As SearchCode, VMain.ManualRefNo As InvoiceNo, 
                    VMain.V_Date As InvoiceDate, VMain.Net_Amount As InvoiceAmount
                    From (" & mQry & ") As VMain
            ...
            ElseIf ReportFrm.FGetText(rowReportType) = "Summary" Then
                mQry = " Select VMain.SaleToPartyMobile As SearchCode, Max(VMain.SaleToPartyName) AS Party, VMain.SaleToPartyMobile AS ContactNo, Count(VMain.DocID) AS TotalBills, 
                    Sum(VMain.Net_Amount) AS TotalBillAmount, 
                    " & IIf(AgL.PubServerName = "", "julianday('now') - julianday(Max(VMain.V_Date))", "DateDiff(DAY,Max(VMain.V_Date),getdate())") & "  AS DaysAfterLastBill
                    From (" & mQry & ") As VMain
                    GROUP By VMain.SaleToPartyMobile
                    Order By Party "
            End If
            ...
            ReportFrm.Text = "Cash Customer Report"
            ReportFrm.ClsRep = Me
            ReportFrm.ReportProcName = "ProcCashCustomerReport"
