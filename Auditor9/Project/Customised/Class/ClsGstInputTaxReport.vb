...
Public Class ClsGstInputTaxReport
    ...
    Dim rowParty As Integer = 7
    Dim rowV_Type As Integer = 8
    Dim rowPartyTaxGroup As Integer = 9
    Dim rowItemTaxGroup As Integer = 10
    Dim rowFilterOnDate As Integer = 11
    ...
    Public Sub Ini_Grid()
        Try
            ...
            ReportFrm.CreateHelpGrid("Party", "Party", FrmRepDisplay.FieldFilterDataType.StringType, FrmRepDisplay.FieldDataType.StringType, "")
            ...
            mQry = "Select 'Doc Date' as Code, 'Doc Date' as Name
                    Union All Select 'Vendor Doc Date' as Code, 'Vendor Doc Date' as Name "
            ReportFrm.CreateHelpGrid("Filter On Date", "Filter On Date", FrmRepDisplay.FieldFilterDataType.StringType, FrmRepDisplay.FieldDataType.SingleSelection, mQry, "Doc Date",,, 300)
            ReportFrm.FilterGrid.Rows(rowHSN).Visible = False 'Hide HSN Row
            ReportFrm.FilterGrid.Rows(rowParty).Visible = False 'Hide HSN Row
    ...
    Public Sub ProcMain(Optional mFilterGrid As AgControls.AgDataGrid = Nothing,
        ...
        Try
            ...
            If mFilterGrid IsNot Nothing And mGridRow IsNot Nothing Then
                If mGridRow.DataGridView.Columns.Contains("Search Code") = True Then
                    If mFilterGrid.Item(GFilter, rowReportType).Value = "Customise Summary" Or
                        ...
                        If ReportFrm.FGetCode(rowSummerise).ToString.Contains("Party") Then
                            mFilterGrid.Item(GFilterCode, rowParty).Value = "'" + mGridRow.Cells("Vendor").Value + "'"
                            mFilterGrid.Item(GFilter, rowParty).Value = mGridRow.Cells("Vendor").Value
                        End If
            ...
            mCondStr = mCondStr & Replace(ReportFrm.GetWhereCondition("H.Vendor", rowParty), "''", "'")
            ...
            mLedgerHeadCondStr = " Where VT.NCat In ('" & Ncat.ExpenseVoucher & "', '" & Ncat.DebitNoteSupplier & "', '" & Ncat.CreditNoteSupplier & "') "
            If ReportFrm.FGetText(rowFilterOnDate) = "Vendor Doc Date" Then
                mLedgerHeadCondStr = mLedgerHeadCondStr & " AND Date(H.VendorDocDate) Between " & AgL.Chk_Date(CDate(ReportFrm.FGetText(rowFromDate)).ToString("s")) & " And " & AgL.Chk_Date(CDate(ReportFrm.FGetText(rowToDate)).ToString("s")) & " "
            Else
            ...
            mLedgerHeadCondStr = mLedgerHeadCondStr & Replace(ReportFrm.GetWhereCondition("H.SubCode", rowParty), "''", "'")
            ...
            mMainQry = " SELECT H.DocID, H.V_Type, Vt.Description as VoucherType, 
                    ...
                    H.Vendor, 
                    (Case When Party.Nature='Cash' Or Party.SubgroupType='" & SubgroupType.RevenuePoint & "' Then Party.Name || ' - ' || IfNull(H.VendorName,'') Else Party.Name End) As VendorName ,                     
            ...
            If ReportFrm.FGetText(0) = "Detail" Then
                ...
                mQry += ", Max(VMain.VendorName) As Party "
            ...
            Else
                ...
                If ReportFrm.FGetCode(rowSummerise).ToString.Contains("Party") Then
                    mGroupColumns += IIf(mGroupColumns = "", "", ",") + "VMain.Vendor,VMain.VendorName"
                    mSelectColumns += IIf(mSelectColumns = "", "", ",") + "VMain.Vendor,VMain.VendorName"
                    mOrderColumns += IIf(mOrderColumns = "", "", ",") + "VMain.VendorName"
                End If
            ...
            ReportFrm.Text = "GST Input Tax Report"
            ReportFrm.ClsRep = Me
