...
Public Class ClsReport
    ...
    Public Sub Ini_Grid()
        Try
            ...
            ReportFrm.CreateHelpGrid("Catalog", "Catalog", FrmRepDisplay.FieldFilterDataType.StringType, FrmRepDisplay.FieldDataType.MultiSelection, mHelpItemTaxGroup)
    ...
    Public Sub ProcSaleReport(Optional mFilterGrid As AgControls.AgDataGrid = Nothing,
        ...
        Try
            ...
            If mFilterGrid IsNot Nothing And mGridRow IsNot Nothing Then
                If mGridRow.DataGridView.Columns.Contains("Search Code") = True Then
                    ...
                    ElseIf mFilterGrid.Item(GFilter, 0).Value = "Catalog Wise Summary" Then
                        mFilterGrid.Item(GFilter, 0).Value = "Doc.Header Wise Detail"
                        mFilterGrid.Item(GFilter, rowCatalog).Value = mGridRow.Cells("Catalog").Value
                        mFilterGrid.Item(GFilterCode, rowCatalog).Value = "'" + mGridRow.Cells("Search Code").Value + "'"
            ...
            mCondStr = mCondStr & ReportFrm.GetWhereCondition("L.Catalog", rowCatalog)
            ...
            If ReportFrm.FGetText(18) <> "All" Then
                mCondStr = mCondStr & " And IfNull(IfNull(I.HSN,IC.HSN),'') = " & AgL.Chk_Text(ReportFrm.FGetText(18)) & " "
            End If
            ...
            mQry = " SELECT H.DocID, H.V_Type, Vt.Description as VoucherType, H.Site_Code, H.Div_Code, Site.Name as Site, Div.Div_Name as Division,
                    ...
                    (Case When H.SaleToParty=H.BillToParty And (Party.Nature='Cash' Or Party.SubgroupType='" & SubgroupType.RevenuePoint & "') Then Party.Name || ' - ' || IfNull(H.SaleToPartyName,'') When H.SaleToParty=H.BillToParty Then Party.Name Else BillToParty.Name || ' - ' || Party.Name End) As SaleToPartyName , 
            ...
            If ReportFrm.FGetText(0) = "Doc.Header Wise Detail" Then
                mQry = " Select VMain.DocId As SearchCode, Max(VMain.Division) as Division, Max(Vmain.Site) as Site, Max(VMain.V_Date) As InvoiceDate, Max(VMain.V_Type) as DocType, Max(VMain.InvoiceNo) As InvoiceNo,
                    Max(VMain.SaleToPartyName) As Party, Max(VMain.SalesTaxGroupParty) As SalesTaxGroupParty, IfNull(Sum(VMain.AmountExDiscount),0) As AmountExDiscount, IfNull(Sum(VMain.Discount),0) As Discount,
            ...
            ElseIf ReportFrm.FGetText(0) = "Sales Representative Wise Summary" Then
                mQry = " Select VMain.SalesRepresentative as SearchCode, Max(VMain.SalesRepresentativeName) As SalesRepresentative, 
                    Count(Distinct Vmain.DocID) as InvoicesCount, Count(Distinct VMain.V_Date) as DaysCount,  Sum(VMain.Qty) as Qty,
                    Sum(VMain.Taxable_Amount) As [Taxable Amount], IfNull(Sum(VMain.TotalTax),0) As TaxAmount, Sum(VMain.Net_Amount) As [Net Amount]
                    From (" & mQry & ") As VMain
            ...
            ElseIf ReportFrm.FGetText(0) = "Catalog Wise Summary" Then
                mQry = " Select VMain.Catalog As SearchCode, Max(VMain.CatalogDesc) As [Item],  
            ...
            ElseIf ReportFrm.FGetText(0) = "Item Wise Summary" Then
                mQry = " Select VMain.Item As SearchCode, Max(VMain.ItemDesc) As [Item],  Round(Sum(VMain.Qty),3) as Qty,
                    IfNull(Sum(VMain.Taxable_Amount),0) As [Taxable Amount], IfNull(Sum(VMain.TotalTax),0) As TaxAmount, IfNull(Sum(VMain.Net_Amount),0) As [Net Amount]
                    From (" & mQry & ") As VMain
                    GROUP By VMain.Item 
                    Order By Max(VMain.ItemDesc)"
            ElseIf ReportFrm.FGetText(0) = "HSN Wise Summary" Then
                mQry = " Select VMain.HSN As SearchCode, VMain.HSN, Max(VMain.ItemCategoryDescription) As [Description],  Round(Sum(VMain.Qty),3) as Qty,
            ...
            ElseIf ReportFrm.FGetText(0) = "Item Group Wise Summary" Then
                mQry = " Select VMain.ItemGroup as SearchCode, Max(VMain.ItemGroupDescription) As [Item Group],  Round(Sum(VMain.Qty),3) as Qty,
                    IfNull(Sum(VMain.Taxable_Amount),0) As [Taxable Amount], IfNull(Sum(VMain.TotalTax),0) As TaxAmount, IfNull(Sum(VMain.Net_Amount),0) As [Net Amount]
                    From (" & mQry & ") As VMain
            ...
            ElseIf ReportFrm.FGetText(0) = "Item Category Wise Summary" Then
                mQry = " Select VMain.ItemCategory as SearchCode, Max(VMain.ItemCategoryDescription) As [Item Category],  Round(Sum(VMain.Qty),3) as Qty,
                    IfNull(Sum(VMain.Taxable_Amount),0) As [Taxable Amount], IfNull(Sum(VMain.TotalTax),0) As TaxAmount, IfNull(Sum(VMain.Net_Amount),0) As [Net Amount]
                    From (" & mQry & ") As VMain
