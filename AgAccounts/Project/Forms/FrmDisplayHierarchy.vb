...
Public Class FrmDisplayHierarchy
    ...
    Private Const GMonthYear As Byte = 16
    ...
    Dim DisplayType_Current As String = ""
    ...
    Private Sub IniGrid(ByVal DspType As ClsStructure.DisplayType)
        ...
        If DspType = ClsStructure.DisplayType.Ledger Then
            ...
            AgCl.AddAgTextColumn(FGMain, "MonthYear", 0, 5, "MonthYear", False, True, False)
    ...
    Sub New()
        ...
        If Trim(DHSMain.StrFromDate) = "" Then
            ...
            DHSMain.StrDivisionCode = "'" + AgL.PubDivCode + "'"
            DHSMain.StrDivisionName = AgL.PubDivName
            DHSMain.StrZeroBalace = "Y"
            DHSMain.StrShowContra = "N"
            DHSMain.DblClosingStock = 0
            DHSMain.StrMonthWise = "N"
        End If
        FSetDTColumn()
    End Sub
    ...
    Private Sub FBalanceSheet_Disp()
        ...
        Try
            ...
            StrCondition1 = " Where Date(LG.V_Date) <= " & AgL.Chk_Date(CDate(DHSMain.StrToDate).ToString("s")) & " And LG.Site_Code In (" & DHSMain.StrSiteCode & ") And LG.DivCode In (" & DHSMain.StrDivisionCode & ") "
    ...
    Private Function FGetTRDDataTable() As DataTable
        ...
        Try
            StrCondition1 = " Where Date(LG.V_Date) <= " & AgL.Chk_Date(CDate(DHSMain.StrToDate).ToString("s")) & " And LG.Site_Code In (" & DHSMain.StrSiteCode & ") And LG.DivCode In (" & DHSMain.StrDivisionCode & ") "
            StrCondition1 += " And Date(LG.V_Date) >= (Case When Ag.GroupNature in ('R','E') Then " & AgL.Chk_Date(CDate(AgL.PubStartDate).ToString("s")) & " Else '1900/Jan/01' End) "
    ...
    Private Sub FDTrailBalance_Disp()
        ...
        Try
            ...
            StrConditionOP = " Where Date(LG.V_Date) < " & AgL.Chk_Date(CDate(DHSMain.StrFromDate).ToString("s")) & " And LG.Site_Code In (" & DHSMain.StrSiteCode & ")  And LG.DivCode In (" & DHSMain.StrDivisionCode & ") "
            StrConditionOP += " And Date(LG.V_Date) >= (Case When Ag.GroupNature in ('R','E') Then '" & CDate(AgL.PubStartDate).ToString("s") & "' Else '1900/Jan/01' End) "
            StrCondition1 = " Where ( Date(LG.V_Date) Between " & AgL.Chk_Date(CDate(DHSMain.StrFromDate).ToString("s")) & " And '" & CDate(DHSMain.StrToDate).ToString("s") & "' ) And LG.Site_Code In (" & DHSMain.StrSiteCode & ")  And LG.DivCode In (" & DHSMain.StrDivisionCode & ") "
            'If DHSMain.StrAcGroup <> "" Then StrConditionAcGroup = " And Sg.GroupCode In  (" & DHSMain.StrAcGroup & ") "
            If DHSMain.StrCostCenter <> "" Then StrConditionAcGroup = " And LG.CostCenter In  (" & DHSMain.StrCostCenter & ") "
            '========== For Detail Section =======
            ...
            If (DblDebit_Total - DblCredit_Total + OpeningTotal) > 0 Then
                FGMain(GGRName, I).Value = "Difference In Trial Balance"
                FGMain(GCredit, I).Value = Format(Math.Abs(DblDebit_Total - DblCredit_Total + OpeningTotal), "0.00")
                DblCredit_Total = DblCredit_Total + Format((DblDebit_Total - DblCredit_Total), "0.00")
                FGMain.Rows(I).DefaultCellStyle.ForeColor = Color.Red
            ElseIf (DblDebit_Total - DblCredit_Total + OpeningTotal) < 0 Then
                FGMain(GGRName, I).Value = "Difference In Trial Balance"
                FGMain(GDebit, I).Value = Format(Math.Abs(DblDebit_Total - DblCredit_Total + OpeningTotal), "0.00")
                DblDebit_Total = DblDebit_Total + Format((DblCredit_Total - DblDebit_Total), "0.00")
                FGMain.Rows(I).DefaultCellStyle.ForeColor = Color.Red
            End If
    ...
    Private Sub FDisplay_Level_Group_MonthWise(ByVal StrForCode As String, ByVal StrForName As String)
        Dim StrCondition1 As String = ""
        Dim DTTemp As DataTable
        Dim DblDebit_Total As Double, DblCredit_Total As Double
        Dim StrConditionZeroBal As String = ""
        ...
        Try
            If FGMain(GMonthYear, FGMain.CurrentRow.Index).Value <> "" And (FGMain(GMonthYear, FGMain.CurrentRow.Index).Value = FGMain(GGRName, FGMain.CurrentRow.Index).Value Or DisplayType_Current = "Ledger") Then
                FDisplay_Level_Group(StrForCode, FGMain(GGRName, I).Tag, FGMain(GMonthYear, FGMain.CurrentRow.Index).Value) : Exit Sub
            End If
            ...
            LblDisplaySite.Text = DHSMain.StrSiteName
            If UCase(DHSMain.StrZeroBalace) = "N" Then StrConditionZeroBal = "Having (Round(IfNull(Sum(LG.AmtDr),0),2)-Round(IfNull(Sum(LG.AmtCr),0),2)) <> 0 "
            'StrCondition1 = " Where (Date(LG.V_Date) Between '" & CDate(DHSMain.StrFromDate).ToString("s") & "' And '" & CDate(DHSMain.StrToDate).ToString("s") & "' ) And LG.Site_Code In (" & DHSMain.StrSiteCode & ")  And LG.DivCode In (" & DHSMain.StrDivisionCode & ") "
            StrCondition1 = " Where Date(LG.V_Date) <= '" & CDate(DHSMain.StrToDate).ToString("s") & "' And LG.Site_Code In (" & DHSMain.StrSiteCode & ")  And LG.DivCode In (" & DHSMain.StrDivisionCode & ") "
            StrCondition1 += " And Date(LG.V_Date) >= (Case When Ag.GroupNature in ('R','E') Then '" & CDate(AgL.PubStartDate).ToString("s") & "' Else '1900/Jan/01' End) "

            'Dim StrConditionOP As String = ""
            'StrConditionOP = " Where Date(LG.V_Date) < '" & CDate(DHSMain.StrFromDate).ToString("s") & "' And LG.Site_Code In (" & DHSMain.StrSiteCode & ")  And LG.DivCode In (" & DHSMain.StrDivisionCode & ") "
            'StrConditionOP += " And Date(LG.V_Date) >= (Case When Ag.GroupNature in ('R','E') Then '" & CDate(AgL.PubStartDate).ToString("s") & "' Else '1900/Jan/01' End) "
            ...
            ''ForOpening
            'StrSQLQuery = "Select '" & StrForCode & "' As GroupCode, Convert(Char(3), LG.V_Date, 0) + '/' + Convert(NVARCHAR,Datepart(YEAR,LG.V_Date)) AS Month,  "
            'StrSQLQuery += "(Case When (IfNull(Sum(LG.AmtDr),0)-IfNull(Sum(LG.AmtCr),0))>0 Then  "
            ...
            '========== For Detail Section =======
            StrSQLQuery = "Select '" & StrForCode & "' As GroupCode, Max(Lg.V_Date) As V_Date, "
            If AgL.PubServerName = "" Then
                StrSQLQuery += "strftime('%m-%Y',LG.V_Date) AS Month,  "
            Else
                StrSQLQuery += " Substring(Convert(NVARCHAR, LG.V_Date,103),4,7) As Month,  "
            End If
            StrSQLQuery += "(Case When (IfNull(Sum(LG.AmtDr),0)-IfNull(Sum(LG.AmtCr),0))>0 Then  "
            StrSQLQuery += "(IfNull(Sum(LG.AmtDr),0)-IfNull(Sum(LG.AmtCr),0)) Else 0.00 End) As AmtDr, "
            StrSQLQuery += "(Case When (IfNull(Sum(LG.AmtCr),0)-IfNull(Sum(LG.AmtDr),0))>0 Then "
            ...
            If AgL.PubServerName = "" Then
                StrSQLQuery += "Group BY strftime('%m-%Y',LG.V_Date) "
            Else
                StrSQLQuery += "Group BY Substring(Convert(NVARCHAR, LG.V_Date,103),4,7) "
            End If

            StrSQLQuery += StrConditionZeroBal

            If AgL.PubServerName = "" Then
                StrSQLQuery += "Order By strftime('%Y',LG.V_Date), strftime('%m',LG.V_Date)  "
            Else
            ...
            DblCredit_Total = 0
            For I = 0 To DTTemp.Rows.Count - 1
                FGMain(GGR_SG, I).Value = StrForCode
                FGMain(GGRCode, I).Value = StrForCode


                FGMain(GMonthYear, I).Value = AgL.XNull(DTTemp.Rows(I).Item("Month"))

                FGMain(GGRName, I).Value = AgL.XNull(DTTemp.Rows(I).Item("Month"))
                FGMain(GGRName, I).Tag = StrForName
                FGMain(GDebit, I).Value = IIf(AgL.VNull(DTTemp.Rows(I).Item("AmtDr")) > 0, Format(AgL.VNull(DTTemp.Rows(I).Item("AmtDr")), "0.00"), "")
                FGMain(GCredit, I).Value = IIf(AgL.VNull(DTTemp.Rows(I).Item("AmtCr")) > 0, Format(AgL.VNull(DTTemp.Rows(I).Item("AmtCr")), "0.00"), "")
                DblDebit_Total = DblDebit_Total + Format(AgL.VNull(DTTemp.Rows(I).Item("AmtDr")), "0.00")
                DblCredit_Total = DblCredit_Total + Format(AgL.VNull(DTTemp.Rows(I).Item("AmtCr")), "0.00")
            ...
            FGMain(GGRName, I + 1).Value = "Total"
            FGMain(GDebit, I + 1).Value = Format(DblDebit_Total, "0.00")
            FGMain.Rows(I + 1).DefaultCellStyle.ForeColor = Color.White
            FGMain.Rows(I + 1).DefaultCellStyle.Font = New Font("Arial", 11, FontStyle.Bold)
    ...
    Private Sub FDisplay_Level_Group(ByVal StrForCode As String, ByVal StrForName As String,
        ...
        Try
            ...
            If MonthYear <> "" Then
                StrCondition1 += " And Date(LG.V_Date) Between " + "'01/" + MonthYear + "' And " + AgL.Chk_Text(AgL.RetMonthEndDate(CDate("01/" + MonthYear)))
            End If
            ...
            For I = 0 To DTTemp.Rows.Count - 1
                ...
                FGMain(GMonthYear, I).Value = MonthYear
    ...
    Private Sub FDisplay_SubGroup(ByVal StrForCode As String, ByVal StrForName As String,
        ...
        Try
            ...
            StrConditionOP = " Where Date(LG.V_Date) < " & AgL.Chk_Date(CDate(DHSMain.StrFromDate).ToString("s")) & " And LG.Site_Code In (" & DHSMain.StrSiteCode & ")  And LG.DivCode In (" & DHSMain.StrDivisionCode & ") "
            StrConditionOP += " And Date(LG.V_Date) >= (Case When Ag.GroupNature in ('R','E') Then '" & CDate(AgL.PubStartDate).ToString("s") & "' Else '1900/Jan/01' End)  "
            StrCondition1 = " Where ( Date(LG.V_Date) Between  " & AgL.Chk_Date(CDate(DHSMain.StrFromDate).ToString("s")) & " And " & AgL.Chk_Date(CDate(DHSMain.StrToDate).ToString("s")) & ") And LG.Site_Code In (" & DHSMain.StrSiteCode & ") And LG.DivCode In (" & DHSMain.StrDivisionCode & ") "
            ...
            StrSQLQuery = StrSQLQuery + "Select	LG.DocId,"
            ...
            StrSQLQuery = StrSQLQuery + "LG.AmtDr,LG.AmtCr,LG.RecID As V_No,"
            ...
            For I = 0 To DTTemp.Rows.Count - 1
                ...
                FGMain(GMonthYear, J).Value = MonthYear
    ...
    Handles BtnClose.Click, BtnSettings.Click, BtnBackWard.Click, BtnPrint.Click
        ...
        Select Case sender.name
            ...
            Case BtnSettings.Name
                ...
                Else
                    If Trim(UCase(DHSMain.StrMonthWise)) = "Y" Then
                        FDisplay_Level_Group_MonthWise(LblDisplay.Tag, LblDisplay.Text)
                    Else
                        FDisplay_Level_Group(LblDisplay.Tag, LblDisplay.Text, FGMain(GMonthYear, FGMain.CurrentRow.Index).Value)
                    End If
                End If
    ...
    Private Sub FBackward()
        ...
        ElseIf IntLevel > 1 Then
            ...
            If Trim(UCase(DHSMain.StrMonthWise)) = "Y" Then
                FDisplay_Level_Group_MonthWise(StrForCode, StrForName)
            Else
                FDisplay_Level_Group(StrForCode, StrForName, FGMain(GMonthYear, FGMain.CurrentRow.Index).Value)
            End If
        End If
    ...
    Public Sub FForward(Optional ByVal IntRow As Integer = 0, Optional ByVal IntCol As Integer = 0, Optional ByVal DspType As ClsStructure.DisplayType = ClsStructure.DisplayType.BalanceSheet)
        ...
        ElseIf UCase(FGMain(GGR_SG, IntRow).Value) = "S" Then
            ...
            If StrForCode <> "" Then
                IntLevel = IntLevel + 1
            End If
            FDisplay_SubGroup(StrForCode, StrForName, FGMain(GMonthYear, FGMain.CurrentRow.Index).Value)
    ...
    Private Sub FOpenForm(ByVal IntRowIndex As Integer)
        ...
        Try
            ...
            If DTRow.Length > 0 Then
                ...
                FrmObj.OpenDocId = Trim(FGMain(GDocId, IntRowIndex).Value)
                FrmObj.Show()
        ...
        Try
            ...
            For I = 0 To RptReg.DataDefinition.FormulaFields.Count - 1
                Select Case CStr(UCase(RptReg.DataDefinition.FormulaFields.Item(I).Name))
                    Case "FDATE"
                        RptReg.DataDefinition.FormulaFields.Item(I).Text = " " & AgL.Chk_Text(CDate(DHSMain.StrFromDate).ToString("s")) & " "
                    Case "TDATE"
                        RptReg.DataDefinition.FormulaFields.Item(I).Text = " " & AgL.Chk_Text(CDate(DHSMain.StrToDate).ToString("s")) & " "
                End Select
            Next
