        ...
        Try
            ...
            If Topctrl1.Mode = "Add" Then
                mQry = "INSERT INTO LedgerM	( " &
                     ...
                     " " & AgL.Chk_Text(LblPrefix.Text.ToString) & ", " & AgL.Chk_Text(CDate(TxtV_Date.Text).ToString("s")) & ", " & AgL.Chk_Text(TxtNarration.Text) & ", " &
                     " '" & AgL.PubUserName & "', '" & AgL.PubUserName & "', " & AgL.Chk_Text(TxtSite_Code.AgSelectedValue) & ", " &
                     " '" & AgL.PubUserName & "'," & AgL.Chk_Text(CDate(AgL.PubLoginDate).ToString("s")) & ",'A')"
                AgL.Dman_ExecuteNonQry(mQry, AgL.GCn, AgL.ECmd)
            Else
                mQry = "Update LedgerM " &
                        " SET " &
                        " V_Date = " & AgL.Chk_Text(CDate(TxtV_Date.Text).ToString("s")) & ", " &
                        " Narration = " & AgL.Chk_Text(TxtNarration.Text) & ", " &
                        " PreparedBY = '" & AgL.PubUserName & "', " &
                        " PostedBY = '" & AgL.PubUserName & "', " &
                        " U_Name = '" & AgL.PubUserName & "', " &
                        " U_EntDt = " & AgL.Chk_Text(CDate(AgL.PubLoginDate).ToString("s")) & ", " &
                        " U_AE = 'E' " &
                ...
                For I = 0 To .RowCount - 1
                    If .Item(Col1_SubCode, I).Value.ToString.Trim <> "" And _
                        ...
                        mQry = "INSERT INTO Ledger(DocId, V_SNO, V_Type, V_No, v_Prefix, DivCode, Site_Code, V_Date, " &
                               ...
                               " " & AgL.Chk_Text(CDate(.Item(Col1_Date, I).Value.ToString).ToString("s")) & "," & AgL.Chk_Text(.AgSelectedValue(Col1_SubCode, I)) & ", " &
                               " " & Val(.Item(Col1_AmtDr, I).Value) & "," & Val(.Item(Col1_AmtCr, I).Value) & ", " & AgL.Chk_Text(.Item(Col1_GroupCode, I).Value) & ", " &
                               " " & AgL.Chk_Text(.Item(Col1_GroupNature, I).Value) & ",'" & AgL.PubUserName & "'," & AgL.Chk_Text(CDate(AgL.PubLoginDate).ToString("s")) & ", " &
