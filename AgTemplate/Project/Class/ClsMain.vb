...
Public Class ClsMain
    ...
    Public Class Charges
        ...
        Public Const TCS As String = "TCS"
...
#Region "Public Help Queries"
    Public Class HelpQueries
        Public Const StockStatus As String = "Select '" & ClsMain.StockStatus.Standard & "' as Code, '" & ClsMain.StockStatus.Standard & "' as Description " &
                                                " Union All Select '" & ClsMain.StockStatus.Rejected & "' as Code, '" & ClsMain.StockStatus.Rejected & "' as Description " &
                                                " Union All Select '" & ClsMain.StockStatus.Damaged & "' as Code, '" & ClsMain.StockStatus.Damaged & "' as Description "

        Public Const BillingType As String = "Select 'Qty' as Code, 'Qty' as Description " &
...
#Region " Structure Update Code "
    ...
    Private Sub FInitialize_ItemType()
        ...
        Try
            If AgL.Dman_Execute(" Select Count(*) From ItemType ", AgL.GCn).ExecuteScalar = 0 Then
                mQry = " INSERT INTO dbo.ItemType (Code, Name) VALUES ('" & ItemType.FinishedMaterial & "', 'Finished Material') " &
                        " INSERT INTO dbo.ItemType (Code, Name) VALUES ('" & ItemType.SemiFinishedMaterial & "', 'Semi Finished Material') " &
                        " INSERT INTO dbo.ItemType (Code, Name) VALUES ('" & ItemType.RawMaterial & "', 'Raw Material') " &
    ...
    Private Sub FInitialize_Subcode()
        ...
        If AgL.Dman_Execute("Select Count(*) From Subgroup Where Subcode = 'PURCHASE'", AgL.GCn).ExecuteScalar = 0 Then
            mQry = "INSERT INTO dbo.SubGroup (SubCode, Site_Code, Div_Code, SiteList, Name, DispName, GroupCode, GroupNature, ManualCode, Nature, U_Name, U_EntDt, U_AE, CommonAc) " &
    ...
    Private Sub Finitialize_AcGroup()
        ...
        If AgL.Dman_Execute("Select Count(*) From AcGroup Where GroupName = 'Capital Account'", AgL.GCn).ExecuteScalar = 0 Then
            mQry = "INSERT INTO dbo.AcGroup (GroupCode, SNo, GroupName, ContraGroupName, GroupUnder, GroupNature, Nature, SysGroup, U_Name, U_EntDt, U_AE, TradingYn, MainGrCode, BlOrd, MainGrLen, ID, Site_Code, GroupNameBiLang, GroupLevel, CurrentCount, CurrentBalance, SubLedYn, AliasYn, GroupHelp, LastYearBalance) " &
    ...
    Private Sub FInitialize_Nature()
        ...
        If AgL.Dman_Execute("Select Count(*) From Nature Where Nature = 'Bank'", AgL.GCn).ExecuteScalar = 0 Then
            mQry = "INSERT INTO dbo.Nature (Nature, Personal_Nature) " &
                 "   VALUES ('Bank', 1) "
            AgL.Dman_ExecuteNonQry(mQry, AgL.GCn)
        End If
    ...
    Private Sub FInitialize_Charges()
        ...
        If AgL.Dman_Execute("Select Count(*) From Charges Where Code = 'ADUTY'", AgL.GCn).ExecuteScalar = 0 Then
            mQry = "INSERT INTO Charges (Code, Description, ManualCode, Div_Code, Site_Code, PreparedBy, U_EntDt, U_AE, ModifiedBy, Edit_Date, UpLoadDate) " &
        ...
        If AgL.Dman_Execute("Select Count(*) From Charges Where Code = 'LV'", AgL.GCn).ExecuteScalar = 0 Then
            mQry = "INSERT INTO dbo.Charges (Code, Description, ManualCode, Div_Code, Site_Code, PreparedBy, U_EntDt, U_AE, ModifiedBy, Edit_Date, UpLoadDate)" &
    ...
    Private Sub CreateView()
        ...
        Try
            mQry = " Create View [ViewSubGroup] As  " &
                    " SELECT SubGroup.SubCode AS AcId, SubGroup.SubCode AS AcCode, SubGroup.Site_Code, " &
                    " SubGroup.SubCode, LEFT(SubGroup.Name COLLATE DATABASE_DEFAULT + IfNull(', ' + City.CityName  COLLATE DATABASE_DEFAULT , ''), 50) AS Name,  " &
                    " SubGroup.GroupCode, SubGroup.GroupNature, " &
                    " SubGroup.Nature, 'N' AS AliasYN, SubGroup.Add1, SubGroup.Add2, SubGroup.Add3, SubGroup.CityCode, " &
                    " SubGroup.PIN, SubGroup.Phone AS PHONEl, AcGroup.MainGrCode AS MainGrCodeS, 'N' AS GAliasYn,  " &
                    " AcGroup.GroupName AS GName, AcGroup.SysGroup AS SysGrp,  " &
                    " LEFT(SubGroup.Name  COLLATE DATABASE_DEFAULT + IfNull(', ' + City.CityName  COLLATE DATABASE_DEFAULT, ''), 50) AS NameWithCity,  City.CityName,  Mobile, Fax, Email,                         AcGroup.GroupName, AcGroup.TradingYn, AcGroup.BlOrd, 'N' AS SubAliasYn, SubGroup.FatherName AS FName,                        SubGroup.CreditLimit, SubGroup.Phone, SubGroup.DueDays, SubGroup.Curr_Bal, IfNull(SubGroup.CommonAc,0) As CommonAc   " &
                    " FROM  SubGroup  " &
...
#Region "Module Functions"
    ...
    Public Shared Function FunRetStock(ByVal bItem As String, ByVal bInternalCode As String, Optional ByVal bCondStr As String = "",
                                       Optional ByVal bGodown As String = "",
                                       Optional ByVal bProcess As String = "",
                                       Optional ByVal bStatus As String = "",
                                       Optional ByVal bV_Date As String = "",
        ...
        Try
            ...
            mQry = "SELECT IfNull(Sum(S.Qty_Rec),0) - IfNull(Sum(S.Qty_Iss),0) " &
...
#End Region
    ...
    Public Shared Function FunCheckStock(ByVal bItem As String, ByVal bInternalCode As String,
                               Optional ByVal bGodown As String = "",
                               Optional ByVal bProcess As String = "",
    ...
    Public Shared Sub ProcStockPost(ByVal bTableName As String, ByVal bStock As ClsMain.StructStock,
                              ByVal bConn As Object,
                              ByVal bCmd As Object)
        Dim mQry$
        ...
        With bStock
            mQry = "INSERT INTO " & bTableName & "(DocID, Sr, V_Type, V_Prefix, V_Date, V_No, RecID, Div_Code, Site_Code, " &
                    ...
                    " " & AgL.Chk_Text(CDate(.V_Date).ToString("s")) & ",	" & Val(.V_No) & ",	" & AgL.Chk_Text(.RecID) & ", " & AgL.Chk_Text(.Div_Code) & ", " &
    ...
    Public Shared Sub PostStructureLineToAccounts(ByVal FGMain As AgStructure.AgCalcGrid, ByVal mNarr As String, ByVal mDocID As String, ByVal mDiv_Code As String,
                                               ...
                                               ByVal Conn As Object, ByVal Cmd As Object, Optional ByVal mCostCenter As String = "")
    ...
    Public Shared Sub ProcPostInDues(ByVal Conn As Object, ByVal Cmd As Object,
    ...
    Public Shared Sub ProcPostInToBeBilled(ByVal Conn As Object, ByVal Cmd As Object, ByVal StructToBeBilled As ToBeBilled)
        Dim mQry$ = ""
    ...
    Public Shared Sub ProcGetPartyAddress(ByVal Party As String, ByRef Address As String, ByRef City As String, ByVal Conn As Object)
        Dim mQry$ = ""
        Dim DtTemp As DataTable = Nothing
        Try
    ...
    Public Shared Function FGetManualRefNo(ByVal FieldName As String, ByVal TableName As String, ByVal V_Type As String, ByVal V_Date As String, ByVal Div_Code As String, ByVal Site_Code As String, Optional ByVal RefType As ManualRefType = ManualRefType.Max) As String
        ...
        If V_Type = "OB" Or V_Type = "WOB" Then
            If CDate(V_Date) < CDate(AgL.PubStartDate) Then
        ...
        End If
            ...
            Dim ManualRefType_VType As String = AgL.XNull(AgL.Dman_Execute("Select ManualRefType 
                            ...
                            Where V_Type = '" & V_Type & "'", IIf(AgL.PubServerName = "", AgL.GCn, AgL.GcnRead)).ExecuteScalar())

        If ManualRefType_VType = "Day Wise" Then
            RefType = ManualRefType.DayWise
        End If

        Select Case RefType
            Case ManualRefType.DayWise
                Dim bDatePrefix As String = CDate(V_Date).ToString("yyMMdd").ToString()
                mQry = "Select IfNull(Max(Convert(Numeric,Replace(Replace(Replace(" & FieldName & ",'-',''),'.',''),'" & bDatePrefix & "',''))),0)+1 From " & TableName & "  WHERE isnumeric(Replace(Replace(" & FieldName & ",'-',''),'.',''))>0 And V_Type = '" & V_Type & "' And Div_Code = '" & Div_Code & "' and Site_Code = '" & Site_Code & "'  And Date(V_Date) = '" & V_Date & "'  AND IsNumeric(" & FieldName & ") > 0 "
                mQry = mQry.Replace("T00:00:00", "")
                FGetManualRefNo = AgL.Dman_Execute(mQry, IIf(AgL.PubServerName = "", AgL.GCn, AgL.GcnRead)).ExecuteScalar
                FGetManualRefNo = bDatePrefix + FGetManualRefNo.ToString.PadLeft(4, "0")

            Case Else
                mQry = "Select Ref_Prefix From Voucher_Prefix With (NoLock) Where V_Type = '" & V_Type & "' And Div_Code = '" & Div_Code & "' And Site_Code = '" & Site_Code & "' And Date(Date_From) = '" & CDate(mStartDate).ToString("s") & "' And Date(Date_To) = '" & CDate(mEndDate).ToString("s") & "'"
                mQry = mQry.Replace("T00:00:00", "")
                mRef_Prefix = AgL.XNull(AgL.Dman_Execute(mQry, IIf(AgL.PubServerName = "", AgL.GCn, AgL.GcnRead)).ExecuteScalar)
                If mRef_Prefix = "" Then
                    If CDate(V_Date) >= CDate("01/Apr/2013") And CDate(V_Date) <= CDate("31/Mar/2014") Then
                        mQry = "Select IfNull(Max(Cast(Replace(Replace(" & FieldName & ",'-',''),'.','') as integer)),0)+1 From " & TableName & " With (NoLock)  WHERE  ABS(Replace(Replace(" & FieldName & ",'-',''),'.',''))>0 And V_Type = '" & V_Type & "' And Div_Code = '" & Div_Code & "' and Site_Code = '" & Site_Code & "'  And Date(V_Date) Between '" & CDate(mStartDate).ToString("s") & "' and  '" & CDate(mEndDate).ToString("s") & "' "
                        mQry = mQry.Replace("T00:00:00", "")
                        FGetManualRefNo = AgL.Dman_Execute(mQry, IIf(AgL.PubServerName = "", AgL.GCn, AgL.GcnRead)).ExecuteScalar
                    ...
                    Else
                        If AgL.PubServerName = "" Then
                            ...
                            mQry = mQry.Replace("T00:00:00", "")
                        Else
                            mQry = "Select IfNull(Max(Cast(Replace(Replace(" & FieldName & ",'-',''),'.','') as BIGINT)),0)+1 From " & TableName & " With (NoLock)  WHERE IsNumeric(Replace(Replace(" & FieldName & ",'-',''),'.',''))>0 And V_Type = '" & V_Type & "' And Div_Code = '" & Div_Code & "' and Site_Code = '" & Site_Code & "'  And Date(V_Date) Between '" & CDate(mStartDate).ToString("s") & "' and  '" & CDate(mEndDate).ToString("s") & "'    "
                            mQry = mQry.Replace("T00:00:00", "")
                        End If
                        FGetManualRefNo = AgL.Dman_Execute(mQry, IIf(AgL.PubServerName = "", AgL.GCn, AgL.GcnRead)).ExecuteScalar
                    End If
                Else
                    If AgL.PubServerName = "" Then
                        mQry = "Select IfNull(Max(Cast(Replace(Replace(Replace(Substring(" & FieldName & "," & mRef_Prefix.Length & ", 20)  ,'-',''),'.',''),'" & mRef_Prefix & "','') as Integer)),0) + 1 From " & TableName & " With (NoLock)  WHERE Abs(Replace(Replace(Replace(" & FieldName & ",'-',''),'.',''),'" & mRef_Prefix & "',''))>0 And V_Type = '" & V_Type & "' And Div_Code = '" & Div_Code & "' and Site_Code = '" & Site_Code & "'  And Date(V_Date) Between '" & CDate(mStartDate).ToString("s") & "' and  '" & CDate(mEndDate).ToString("s") & "' And Left(" & FieldName & "," & mRef_Prefix.Length & ")='" & mRef_Prefix & "'   "
                        mQry = mQry.Replace("T00:00:00", "")
                    Else
                        mQry = "Select IfNull(Max(Cast(Replace(Replace(Substring(" & FieldName & "," & mRef_Prefix.Length & ", 20),'-',''),'.','') as integer)),0)+1 From " & TableName & " With (NoLock)  WHERE IsNumeric(Replace(Replace(" & FieldName & ",'-',''),'.',''))>0 And V_Type = '" & V_Type & "' And Div_Code = '" & Div_Code & "' and Site_Code = '" & Site_Code & "'  And Date(V_Date) Between '" & CDate(mStartDate).ToString("s") & "' and  '" & CDate(mEndDate).ToString("s") & "'    "
                        mQry = mQry.Replace("T00:00:00", "")
                    End If
                    FGetManualRefNo = AgL.Dman_Execute(mQry, IIf(AgL.PubServerName = "", AgL.GCn, AgL.GcnRead)).ExecuteScalar

                    mQry = "Select Ref_PadLength From Voucher_Prefix With (NoLock) Where V_Type = '" & V_Type & "' And Div_Code = '" & Div_Code & "' And Site_Code = '" & Site_Code & "' And Date(Date_From) = '" & CDate(mStartDate).ToString("s") & "' And Date(Date_To) = '" & CDate(mEndDate).ToString("s") & "'"
                    mQry = mQry.Replace("T00:00:00", "")
                    mRef_PadLength = AgL.VNull(AgL.Dman_Execute(mQry, IIf(AgL.PubServerName = "", AgL.GCn, AgL.GcnRead)).ExecuteScalar)
    ...
    Public Shared Function FCheckDuplicateRefNo(ByVal FieldName As String, ByVal TableName As String,
                                          ...
                                          ByVal InternalCode As String, Optional SilentMode As Boolean = False) As Boolean
        Dim mQry$ = ""
        ...
        If AgL.StrCmp(EntryMode, "Add") Then
            mQry = " SELECT COUNT(*) " &
                    ...
                    " And Date(V_Date) Between " & AgL.Chk_Date(mStartDate) & " and  " & AgL.Chk_Date(mEndDate) & "  "
            mQry = mQry.Replace("T00:00:00", "")
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then
                FCheckDuplicateRefNo = False
                If Not SilentMode Then MsgBox("Reference No. Already Exists")
            End If
        Else
            mQry = " SELECT COUNT(*) " &
                    " FROM " & TableName & " " &
    ...
    Public Shared Sub UpdateVoucherCounter(ByVal mDocId As String, ByVal mDate As Date, ByVal mConn As SqlClient.SqlConnection,
                                    ByVal mCmd As SqlClient.SqlCommand, ByVal mDiv_Code As String, ByVal mSite_Code As String,
                                    Optional ByVal mDelete_Record As Boolean = False, Optional ByVal mComp_Code As String = "")
        Dim RdTemp As SqlClient.SqlDataReader = Nothing
        Dim mVno As Long
        Dim mVType As String
        Dim mVPrefix As String
        Dim CondStr As String
    ...
    Public Shared Sub ProcExecuteQry_FromTextFile(ByVal Conn As Object, ByVal Cmd As Object)
    ...
    Public Shared Sub FSaveAlerts(ByVal SearchCode As String, ByVal SearchField As String, ByVal SearchTable As String,
        ...
        Dim mQry$ = ""
        Dim mConn As New Object
        Dim mCmd As New Object
    ...
    Public Shared Function FIsApplyVTypePermission(ByVal mUser As String, ByVal mNCat As String)
        FIsApplyVTypePermission = AgL.Dman_Execute("SELECT (Case When Count(*) > 0 Then 1 Else 0 End) " &
                                                   "FROM User_VType_Permission H " &
                                                   "LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type  " &
                                                   "WHERE H.Div_Code ='" & AgL.PubDivCode & "' " &
                                                   "And H.Site_Code = '" & AgL.PubSiteCode & "' " &
                                                   "AND Vt.NCat In ('" & mNCat & "')  " &
                                                   "AND H.UserName ='" & mUser & "'", AgL.GcnMain).ExecuteScalar
    ...
    Public Shared Function FRetImportForm(ByVal OwnerWindow As Form, ByVal V_Type As String) As Form
        ...
        Try
            mQry = " Select ImportMnuName,ImportMnuText,ImportMnuAttachedInModule " &
                    " From Voucher_Type_Settings Where V_Type = '" & V_Type & "' " &
    ...
    '-------------------------------For Log Management-----------------------------------------
    Public Shared Sub FCreateObjectOfForm(ByRef mFrmObjBeforeModification As Object, ByRef mFrmObjNew As Object)
        Dim I As Integer = 0
        Dim J As Integer = 0

        mFrmObjBeforeModification.Name = mFrmObjNew.Name
        For I = 0 To mFrmObjNew.Controls.Count - 1
            If mFrmObjNew.Controls(I).GetType.ToString = GetType(TabControl).ToString Then
                Dim ChildObj As New TabControl
                ChildObj.Name = CType(mFrmObjNew.Controls(I), TabControl).Name
                FCreateObjectOfForm(ChildObj, CType(mFrmObjNew, Control).Controls(I))
                mFrmObjBeforeModification.Controls.Add(ChildObj)
            ElseIf mFrmObjNew.Controls(I).GetType.ToString = GetType(TabPage).ToString Then
                Dim ChildObj As New TabPage
                ChildObj.Name = CType(mFrmObjNew.Controls(I), TabPage).Name
                FCreateObjectOfForm(ChildObj, CType(mFrmObjNew, Control).Controls(I))
                mFrmObjBeforeModification.Controls.Add(ChildObj)
            Else
                If mFrmObjNew.Controls(I).GetType.ToString = GetType(AgControls.AgTextBox).ToString Then
                    Dim ChildObj As New AgControls.AgTextBox
                    ChildObj.Name = CType(mFrmObjNew.Controls(I), AgControls.AgTextBox).Name
                    ChildObj.Text = CType(mFrmObjNew.Controls(I), AgControls.AgTextBox).Text
                    mFrmObjBeforeModification.Controls.Add(ChildObj)
                End If


                If (mFrmObjNew.Controls(I).GetType.ToString = GetType(AgControls.AgDataGrid).ToString Or
                    mFrmObjNew.Controls(I).GetType.ToString = GetType(AgStructure.AgCalcGrid).ToString) And
                    mFrmObjNew.Controls(I).Name <> "HelpDg" Then
                    Dim ChildObj As New AgControls.AgDataGrid
                    If mFrmObjNew.Controls(I).GetType.ToString = GetType(AgStructure.AgCalcGrid).ToString Then
                        ChildObj = New AgStructure.AgCalcGrid
                    End If
                    ChildObj.Name = CType(mFrmObjNew.Controls(I), AgControls.AgDataGrid).Name

                    If ChildObj.Columns.Count = 0 Then
                        For Each dgvc As DataGridViewColumn In CType(mFrmObjNew.Controls(I), AgControls.AgDataGrid).Columns
                            ChildObj.Columns.Add(TryCast(dgvc.Clone(), DataGridViewColumn))
                        Next
                    End If

                    Dim row As New DataGridViewRow()

                    For J = 0 To CType(mFrmObjNew.Controls(I), AgControls.AgDataGrid).Rows.Count - 1
                        row = DirectCast(CType(mFrmObjNew.Controls(I), AgControls.AgDataGrid).Rows(J).Clone(), DataGridViewRow)
                        Dim intColIndex As Integer = 0
                        For Each cell As DataGridViewCell In CType(mFrmObjNew.Controls(I), AgControls.AgDataGrid).Rows(J).Cells
                            row.Cells(intColIndex).Value = AgL.XNull(cell.Value).ToString
                            intColIndex += 1
                        Next
                        ChildObj.Rows.Add(row)
                    Next

                    ChildObj.Tag = CType(mFrmObjNew.Controls(I), AgControls.AgDataGrid).Tag
                    mFrmObjBeforeModification.Controls.Add(ChildObj)
                End If
            End If
        Next
    End Sub
    Public Shared Sub FCreateLogForEdit(ByRef mFrmObjBeforeModification As Object, ByRef mFrmObjNew As Object,
                        ByRef mLogText As String)
        Dim I As Integer = 0
        Dim J As Integer = 0
        Dim K As Integer = 0
        Dim L As Integer = 0
        ...
        Const ChargeValueColumnIndex As Integer = 16
        Const ChargeColumnIndex As Integer = 0

        Const ColHead As String = "Head"
        ...
        Try
            If mFrmObjBeforeModification IsNot Nothing Then
                For I = 0 To mFrmObjBeforeModification.Controls.Count - 1
                    For J = 0 To mFrmObjNew.Controls.Count - 1
                        If mFrmObjBeforeModification.Controls(I).Name = mFrmObjNew.Controls(J).Name Then
                            If mFrmObjNew.Controls(J).GetType.ToString = GetType(TabControl).ToString And
                                    mFrmObjBeforeModification.Controls(I).GetType.ToString = GetType(TabControl).ToString Then
                                FCreateLogForEdit(mFrmObjBeforeModification.Controls(I), mFrmObjNew.Controls(J), mLogText)
                            ElseIf mFrmObjNew.Controls(J).GetType.ToString = GetType(TabPage).ToString And
                                    mFrmObjBeforeModification.Controls(I).GetType.ToString = GetType(TabPage).ToString Then
                                FCreateLogForEdit(mFrmObjBeforeModification.Controls(I), mFrmObjNew.Controls(J), mLogText)
                            Else
                                If mFrmObjNew.Controls(J).GetType.ToString = GetType(AgControls.AgTextBox).ToString And
                                    mFrmObjBeforeModification.Controls(I).GetType.ToString = GetType(AgControls.AgTextBox).ToString Then
                                    If CType(mFrmObjBeforeModification.Controls(I), AgControls.AgTextBox).Text <> CType(mFrmObjNew.Controls(J), AgControls.AgTextBox).Text Then
                                        If mLogText = "" Then mLogText += "<Log>" + vbCrLf : mLogText += TabStr_1 + "<" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew.Controls(J), Control).FindForm.Text) + ">" + vbCrLf
                                        mLogText += TabStr_2 + "<" + CType(mFrmObjNew.Controls(J), AgControls.AgTextBox).Name + ">" + vbCrLf
                                        mLogText += TabStr_3 + "<New>" + CType(mFrmObjNew.Controls(J), AgControls.AgTextBox).Text + "</New>" + vbCrLf
                                        mLogText += TabStr_3 + "<Old>" + CType(mFrmObjBeforeModification.Controls(I), AgControls.AgTextBox).Text + "</Old>" + vbCrLf
                                        mLogText += TabStr_2 + "</" + CType(mFrmObjNew.Controls(J), AgControls.AgTextBox).Name + ">" + vbCrLf
                                    End If
                                End If

                                If mFrmObjNew.Controls(J).GetType.ToString = GetType(AgControls.AgDataGrid).ToString And
                                        mFrmObjBeforeModification.Controls(I).GetType.ToString = GetType(AgControls.AgDataGrid).ToString And
                                        mFrmObjNew.Controls(J).Name <> "HelpDg" And
                                        mFrmObjNew.Controls(J).Name <> "" And
                                        AgL.XNull(mFrmObjBeforeModification.Controls(I).Tag) <> "VerticalGrid" Then
                                    For K = 0 To CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Rows.Count - 1
                                        For L = 0 To CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Columns.Count - 1
                                            If CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Columns(L).Visible = True Then
                                                If K <= CType(mFrmObjBeforeModification.Controls(I), AgControls.AgDataGrid).Rows.Count - 1 Then
                                                    If FCompareValues(AgL.XNull(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Item(L, K).Value).ToString,
                                                                  AgL.XNull(CType(mFrmObjBeforeModification.Controls(I), AgControls.AgDataGrid).Item(L, K).Value).ToString) = False Then
                                                        If AgL.XNull(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Item(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).AgMandatoryColumn, K).Value).ToString <> "" Then
                                                            'If mLogText = "" Then mLogText += "<Log>" + vbCrLf : mLogText += TabStr_1 + "<" + FRemoveSpecialCharactersForXML(mFrmObjNew.Text) + ">" + vbCrLf
                                                            If mLogText = "" Then mLogText += "<Log>" + vbCrLf : mLogText += TabStr_1 + "<" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew.Controls(J), Control).FindForm.Text) + ">" + vbCrLf
                                                            If Not mLogText.Contains("<" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Name) + ">") Then mLogText += TabStr_2 + "<" + CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Name + ">" + vbCrLf
                                                            If Not mLogText.Contains("<" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Name) + "-" + (K + 1).ToString + ">") Then mLogText += TabStr_3 + "<" + CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Name + "-" + (K + 1).ToString + ">" + vbCrLf
                                                            mLogText += TabStr_4 + "<" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Columns(L).Name) + ">" + vbCrLf
                                                            mLogText += TabStr_5 + "<New>" + AgL.XNull(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Item(L, K).Value).ToString + "</New>" + vbCrLf
                                                            mLogText += TabStr_5 + "<Old>" + AgL.XNull(CType(mFrmObjBeforeModification.Controls(I), AgControls.AgDataGrid).Item(L, K).Value).ToString + "</Old>" + vbCrLf
                                                            mLogText += TabStr_4 + "</" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Columns(L).Name) + ">" + vbCrLf
                                                        End If
                                                    End If
                                                Else
                                                    If AgL.XNull(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Item(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).AgMandatoryColumn, K).Value).ToString <> "" Then
                                                        ...
                                                        If Not mLogText.Contains("<" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Name) + "-" + (K + 1).ToString + ">") Then mLogText += TabStr_3 + "<" + CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Name + "-" + (K + 1).ToString + ">" + vbCrLf
                                                        mLogText += TabStr_4 + "<" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Columns(L).Name) + ">" + vbCrLf
                                                        mLogText += TabStr_5 + "<Value>" + AgL.XNull(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Item(L, K).Value).ToString + "</Value>" + vbCrLf
                                                        mLogText += TabStr_4 + "</" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Columns(L).Name) + ">" + vbCrLf
                                                    End If
                                                End If
                                            End If
                                        Next
                                        If mLogText.Contains("<" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Name) + "-" + (K + 1).ToString + ">") Then mLogText += TabStr_3 + "</" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Name) + "-" + (K + 1).ToString + ">" + vbCrLf
                                    Next
                                    If mLogText.Contains("<" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Name) + ">") Then mLogText += TabStr_2 + "</" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Name) + ">" + vbCrLf
                                End If

                                If mFrmObjNew.Controls(J).GetType.ToString = GetType(AgControls.AgDataGrid).ToString And
                                        mFrmObjBeforeModification.Controls(I).GetType.ToString = GetType(AgControls.AgDataGrid).ToString And
                                        AgL.XNull(mFrmObjBeforeModification.Controls(I).Tag) = "VerticalGrid" Then
                                    For K = 0 To CType(mFrmObjBeforeModification.Controls(I), AgControls.AgDataGrid).Rows.Count - 1
                                        For L = 0 To CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Rows.Count - 1
                                            If AgL.XNull(CType(mFrmObjBeforeModification.Controls(I), AgControls.AgDataGrid).Item(ColHead, K).Value).ToString =
                                                    AgL.XNull(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Item(ColHead, L).Value).ToString Then
                                                If FCompareValues(AgL.XNull(CType(mFrmObjBeforeModification.Controls(I), AgControls.AgDataGrid).Item(ColValue, K).Value).ToString,
                                                              AgL.XNull(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Item(ColValue, L).Value).ToString) = False Then
                                                    'If mLogText = "" Then mLogText += "<Log>" + vbCrLf : mLogText += TabStr_1 + "<" + FRemoveSpecialCharactersForXML(mFrmObjNew.Text) + ">" + vbCrLf
                                                    If mLogText = "" Then mLogText += "<Log>" + vbCrLf : mLogText += TabStr_1 + "<" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew.Controls(J), Control).FindForm.Text) + ">" + vbCrLf
                                                    If Not mLogText.Contains("<" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Name) + ">") Then mLogText += TabStr_2 + "<" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Name) + ">" + vbCrLf
                                                    mLogText += TabStr_4 + "<" + FRemoveSpecialCharactersForXML(AgL.XNull(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Item(ColHead, L).Value).ToString) + ">" + vbCrLf
                                                    mLogText += TabStr_5 + "<New>" + AgL.XNull(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Item(ColValue, L).Value).ToString + "</New>" + vbCrLf
                                                    mLogText += TabStr_5 + "<Old>" + AgL.XNull(CType(mFrmObjBeforeModification.Controls(I), AgControls.AgDataGrid).Item(ColValue, K).Value).ToString + "</Old>" + vbCrLf
                                                    mLogText += TabStr_4 + "</" + FRemoveSpecialCharactersForXML(AgL.XNull(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Item(ColHead, L).Value).ToString) + ">" + vbCrLf
                                                End If
                                            End If
                                        Next
                                ...
                                If mFrmObjNew.Controls(J).GetType.ToString = GetType(AgStructure.AgCalcGrid).ToString And
                                        mFrmObjBeforeModification.Controls(I).GetType.ToString = GetType(AgStructure.AgCalcGrid).ToString Then
                                    For K = 0 To CType(mFrmObjBeforeModification.Controls(I), AgControls.AgDataGrid).Rows.Count - 1
                                        For L = 0 To CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Rows.Count - 1
                                            If AgL.XNull(CType(mFrmObjBeforeModification.Controls(I), AgControls.AgDataGrid).Item(ChargeColumnIndex, K).Value).ToString =
                                                    AgL.XNull(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Item(ChargeColumnIndex, L).Value).ToString Then
                                                If AgL.VNull(CType(mFrmObjBeforeModification.Controls(I), AgControls.AgDataGrid).Item(ChargeValueColumnIndex, K).Value).ToString <>
                                                        AgL.VNull(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Item(ChargeValueColumnIndex, L).Value).ToString Then
                                                    'If mLogText = "" Then mLogText += "<Log>" + vbCrLf : mLogText += TabStr_1 + "<" + FRemoveSpecialCharactersForXML(mFrmObjNew.Text) + ">" + vbCrLf
                                                    If mLogText = "" Then mLogText += "<Log>" + vbCrLf : mLogText += TabStr_1 + "<" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew.Controls(J), Control).FindForm.Text) + ">" + vbCrLf
                                                    If Not mLogText.Contains("<" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Name) + ">") Then mLogText += TabStr_2 + "<" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Name) + ">" + vbCrLf
                                                    mLogText += TabStr_4 + "<" + FRemoveSpecialCharactersForXML(AgL.XNull(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Item(ChargeColumnIndex, L).Value).ToString) + ">" + vbCrLf
                                                    mLogText += TabStr_5 + "<New>" + AgL.XNull(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Item(ChargeValueColumnIndex, L).Value).ToString + "</New>" + vbCrLf
                                                    mLogText += TabStr_5 + "<Old>" + AgL.XNull(CType(mFrmObjBeforeModification.Controls(I), AgControls.AgDataGrid).Item(ChargeValueColumnIndex, K).Value).ToString + "</Old>" + vbCrLf
                                                    mLogText += TabStr_4 + "</" + FRemoveSpecialCharactersForXML(AgL.XNull(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Item(ChargeColumnIndex, L).Value).ToString) + ">" + vbCrLf
                                                End If
                                            End If
                                        Next
                    ...
                    Next
                Next
                'If mLogText <> "" And Not mLogText.Contains("</" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew, Control).FindForm.Text) + ">" + vbCrLf + "</Log>") Then mLogText += TabStr_1 + "</" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew, Control).FindForm.Text) + ">" + vbCrLf : mLogText += "</Log>"
                If mLogText <> "" And
                        mFrmObjNew.GetType.ToString <> GetType(TabControl).ToString And
                        mFrmObjNew.GetType.ToString <> GetType(TabPage).ToString And
                        Not mLogText.Contains("</" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew, Control).FindForm.Text) + ">" + vbCrLf + "</Log>") Then
                    mLogText += TabStr_1 + "</" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew, Control).FindForm.Text) + ">" + vbCrLf
                    mLogText += "</Log>"
                End If
            End If
        Catch ex As Exception
            MsgBox(ex.Message + " Error In Log Creation.")
        End Try
    End Sub
    Public Shared Sub FCreateLogForDelete(ByRef mFrmObjNew As Object, ByRef mLogText As String)
        Dim J As Integer = 0
        Dim K As Integer = 0
        Dim L As Integer = 0

        Const ChargeColumnIndex As Integer = 16
        Const ChargeValueColumnIndex As Integer = 0
        ...
        Try
            For J = 0 To mFrmObjNew.Controls.Count - 1
                If mFrmObjNew.Controls(J).GetType.ToString = GetType(TabControl).ToString Then
                    FCreateLogForDelete(mFrmObjNew.Controls(J), mLogText)
                End If

                If mFrmObjNew.Controls(J).GetType.ToString = GetType(TabPage).ToString Then
                    FCreateLogForDelete(mFrmObjNew.Controls(J), mLogText)
                End If

                If mFrmObjNew.Controls(J).GetType.ToString = GetType(AgControls.AgTextBox).ToString Then
                    If CType(mFrmObjNew.Controls(J), AgControls.AgTextBox).Name <> "" Then
                        'If mLogText = "" Then mLogText += "<Log>" + vbCrLf : mLogText += TabStr_1 + "<" + FRemoveSpecialCharactersForXML(mFrmObjNew.Text) + ">" + vbCrLf
                        If mLogText = "" Then mLogText += "<Log>" + vbCrLf : mLogText += TabStr_1 + "<" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew.Controls(J), Control).FindForm.Text) + ">" + vbCrLf
                        mLogText += TabStr_2 + "<" + CType(mFrmObjNew.Controls(J), AgControls.AgTextBox).Name + ">" + vbCrLf
                        mLogText += TabStr_3 + "<Value>" + CType(mFrmObjNew.Controls(J), AgControls.AgTextBox).Text + "</Value>" + vbCrLf
                        mLogText += TabStr_2 + "</" + CType(mFrmObjNew.Controls(J), AgControls.AgTextBox).Name + ">" + vbCrLf
                    End If
                End If
                ...
                If mFrmObjNew.Controls(J).GetType.ToString = GetType(AgControls.AgDataGrid).ToString And
                        mFrmObjNew.Controls(J).Name <> "HelpDg" And
                        mFrmObjNew.Controls(J).Name <> "" And
                        AgL.XNull(mFrmObjNew.Controls(J).Tag) <> "VerticalGrid" Then
                    For K = 0 To CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Rows.Count - 1
                        For L = 0 To CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Columns.Count - 1
                            If AgL.XNull(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Item(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).AgMandatoryColumn, K).Value).ToString <> "" Then
                                'If mLogText = "" Then mLogText += "<Log>" + vbCrLf : mLogText += TabStr_1 + "<" + FRemoveSpecialCharactersForXML(mFrmObjNew.Text) + ">" + vbCrLf
                                If mLogText = "" Then mLogText += "<Log>" + vbCrLf : mLogText += TabStr_1 + "<" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew.Controls(J), Control).FindForm.Text) + ">" + vbCrLf
                                If Not mLogText.Contains("<" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Name) + ">") Then mLogText += TabStr_2 + "<" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Name) + ">" + vbCrLf
                                If Not mLogText.Contains("<" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Name) + "-" + (K + 1).ToString + ">") Then mLogText += TabStr_3 + "<" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Name) + "-" + (K + 1).ToString + ">" + vbCrLf
                                mLogText += TabStr_4 + "<" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Columns(L).Name) + ">" + vbCrLf
                                mLogText += TabStr_5 + "<Value>" + AgL.XNull(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Item(L, K).Value).ToString + "</Value>" + vbCrLf
                                mLogText += TabStr_4 + "</" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Columns(L).Name) + ">" + vbCrLf
                            End If
                ...
                If mFrmObjNew.Controls(J).GetType.ToString = GetType(AgControls.AgDataGrid).ToString And
                    ...
                    For L = 0 To CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Rows.Count - 1
                        ...
                        If Not mLogText.Contains("<" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Name) + ">") Then mLogText += TabStr_2 + "<" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Name) + ">" + vbCrLf
                        mLogText += TabStr_4 + "<" + FRemoveSpecialCharactersForXML(AgL.XNull(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Item(ColHead, L).Value).ToString) + ">" + vbCrLf
                        mLogText += TabStr_5 + "<Value>" + AgL.XNull(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Item(ColValue, L).Value).ToString + "</Value>" + vbCrLf
                        mLogText += TabStr_4 + "</" + FRemoveSpecialCharactersForXML(AgL.XNull(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Item(ColHead, L).Value).ToString) + ">" + vbCrLf
                    Next
                    If mLogText.Contains("<" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Name) + ">") Then mLogText += TabStr_2 + "</" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Name) + ">" + vbCrLf
                End If
                ...
                If mFrmObjNew.Controls(J).GetType.ToString = GetType(AgStructure.AgCalcGrid).ToString And
                    mFrmObjNew.Controls(J).Name <> "" Then
                    For L = 0 To CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Rows.Count - 1
                        'If mLogText = "" Then mLogText += "<Log>" + vbCrLf : mLogText += TabStr_1 + "<" + FRemoveSpecialCharactersForXML(mFrmObjNew.Text) + ">" + vbCrLf
                        If mLogText = "" Then mLogText += "<Log>" + vbCrLf : mLogText += TabStr_1 + "<" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew.Controls(J), Control).FindForm.Text) + ">" + vbCrLf
                        If Not mLogText.Contains("<" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Name) + ">") Then mLogText += TabStr_2 + "<" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Name) + ">" + vbCrLf
                        mLogText += TabStr_4 + "<" + FRemoveSpecialCharactersForXML(AgL.XNull(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Item(ChargeValueColumnIndex, L).Value).ToString) + ">" + vbCrLf
                        mLogText += TabStr_5 + "<Value>" + AgL.XNull(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Item(ChargeColumnIndex, L).Value).ToString + "</Value>" + vbCrLf
                        mLogText += TabStr_4 + "</" + FRemoveSpecialCharactersForXML(AgL.XNull(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Item(ChargeValueColumnIndex, L).Value).ToString) + ">" + vbCrLf
                    Next
                    If mLogText.Contains("<" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Name) + ">") Then mLogText += TabStr_2 + "</" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew.Controls(J), AgControls.AgDataGrid).Name) + ">" + vbCrLf
                End If
            Next
            'If mLogText <> "" Then mLogText += TabStr_1 + "</" + FRemoveSpecialCharactersForXML(mFrmObjNew.Text) + ">" + vbCrLf : mLogText += "</Log>"
            If mLogText <> "" And
                    mFrmObjNew.GetType.ToString <> GetType(TabControl).ToString And
                    mFrmObjNew.GetType.ToString <> GetType(TabPage).ToString And
                    Not mLogText.Contains("</" + FRemoveSpecialCharactersForXML(CType(mFrmObjNew, Control).FindForm.Text) + ">" + vbCrLf + "</Log>") Then
        ...
        End Try
    End Sub
    Private Shared Function FRemoveSpecialCharactersForXML(StrValue As String)
        'CAST('<Name><FName>Carol</FName><LName>Elliot</LName></Name>'  AS XML)
        FRemoveSpecialCharactersForXML = StrValue.Replace(" ", "").Replace("~", "").Replace("`", "").Replace("!", "").
    ...
    Private Shared Function FCompareValues(Value1 As String, Value2 As String) As Boolean
        If IsNumeric(Value1) And IsNumeric(Value2) Then
            If Val(AgL.VNull(Value1)) = Val(AgL.VNull(Value2)) Then
                Return True
            Else
                Return False
            End If
        Else
            If AgL.XNull(Value1) = AgL.XNull(Value2) Then
                Return True
            Else
                Return False
            End If
    ...
    '-------------------------------End For Log Management-----------------------------------------


    Public Shared Function FormatDate(strDate As String) As String
        Dim mDate As String = ""
        Try
            If AgL.XNull(strDate) <> "" Then
                mDate = Format(CDate(strDate), "dd/MMM/yyyy")
            End If
        Catch ex As Exception
            MsgBox(ex.Message)
        Finally
    ...
    Public Shared Function FGetSettings_Template(FieldName As String, SettingType As String, Div_Code As String, Site_Code As String, Category As String, NCat As String, VoucherType As String, Process As String, SettingGroup As String) As String
        ...
        If DrSetting.Length > 0 Then
            FGetSettings_Template = AgL.XNull(DrSetting(0)("Value"))
        Else
            FGetSettings_Template = ""
        End If
    End Function

    Public Class SettingFields_Template
        Public Shared MaintainLogForEditYn As String = "Maintain Log For Edit Y/N"
        Public Shared MaintainLogForDeleteYn As String = "Maintain Log For Delete Y/N"
    End Class
End Class